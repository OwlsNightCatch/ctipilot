...
8. **Relevance discipline (no hardcoded volume).** Entry volume follows a strict relevance/actionability gate, not a count — there is no per-run, per-day, or rolling-24 h target or ceiling. The window carries exactly the entries that clear the gate, however few or many; every entry must earn its place, and the reader is never overflooded with marginal items. More runs mean lower latency, never more content (dedup guarantees it). A run that finds nothing publishes only its run record — a healthy outcome.
