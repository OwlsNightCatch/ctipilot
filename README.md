...
7. **No repetition across runs.** Every run builds a prior-coverage index over the last 14 days of entries — **including entries earlier runs published the same day** — which the main agent loads in full (every brief's `summary`) into context, checking each candidate against them all; coverage older than 14 days is caught by the store-wide CVE index. A covered story returns solely as a new entry with `update_of: <original entry id>` carrying a material delta; the mechanical gate FAILs CVE-level duplicates.
...
3. **Phase 2 — Verification & triage.** URL spot-checks, two-source / carve-out rule, fake-news guard, CVE verification on NVD/MITRE, the dedup decision (new entry vs `update_of` vs drop), recency re-check, the relevance/actionability gate (drop anything that doesn't clear it, no matter the count), priority assignment.
...
8. **Phase 5.7 — Verifier loop (gatekeeper).** [`cti-verification`](.claude/agents/cti-verification.md) reads the run's entries + record cold; a **confirmed CLEAN — two consecutive CLEAN verdicts from two different models — gates publish** (a first CLEAN triggers a confirmation pass on the other model). NEEDS_FIXES → remediate, re-run the gate, re-spawn fresh. **8-iteration cap with model rotation** — odd iterations Opus, even iterations the Sonnet-pinned [`cti-verification-alt`](.claude/agents/cti-verification-alt.md) — and a documented fail-open at the cap. Verification can drop entries; it never blocks the run record.
...
- **Adversarial verification loop.** An independent cold-reader sub-agent (model-rotated across iterations) re-fetches every URL and traces every claim before publish; the commit is gated on two consecutive CLEAN verdicts from two different models.
