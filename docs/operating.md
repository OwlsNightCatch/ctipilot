...
7. **`JINA_API_KEYS` / `JINA_API_KEY` — reader-proxy API key pool (recommended).** The fetch bridge's jina reader transport (`tools/fetch_source.py jina <URL>` and the `url` auto-fallback — tier 3 of the fetch ladder) authenticates every `r.jina.ai` request with `Authorization: Bearer <key>` when at least one key is configured in the routine container. With a key the connector gets a dedicated rate limit and the `X-Engine: browser` rendering tier, which recovers article bodies the default engine cannot (e.g. heise.de per-article pages behind the TollBit gate — recovered 2026-07-12). Without any live key the reader still works anonymously (shared rate limit, no browser engine), so the variables are recommended, not required.
   - **Multiple keys, automatic rotation:** `JINA_API_KEYS` takes one or more keys separated by commas / semicolons / whitespace (listed order = spend order); the original single-key `JINA_API_KEY` still works and is appended after the list (it may itself carry a separated list). A key answering HTTP 402 (balance exhausted) or 401 (invalid/revoked) is skipped for the rest of the process and the next key takes over; when **no** live key remains the connector **automatically falls back to the anonymous free tier** — an exhausted pool degrades fetch fidelity, never availability.
