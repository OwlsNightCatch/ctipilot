...
> **Prompt version:** v3.29 — bump in `prompts/CHANGELOG.md` whenever you edit this file. Carry the version through to the run record (`prompt_version` in `runs/<date>/<run-id>.md`). The routine should print this banner at the start of the run so the operator can verify which version executed.
>
...
- **Use the cheapest transport that returns the full body — jina last** (§ [`.claude/agents/cti-research.md`](../.claude/agents/cti-research.md) Fetch tooling reference): prefer `url <URL>` (`python3 tools/fetch_source.py url <URL>` — full raw body, auto-reader-fallback on a challenge/403) or the host's structured recipe; when the raw HTML is heavy, write it to `work/<run-id>/` and extract the passages you need on disk (grep / a short python snippet) so the bulk never enters your context — the 2026-07-18 run proved this path. Force the **jina reader** (`jina <URL>`) only for hosts the direct transport cannot read or whose source record pins `fetch_method: jina` (heise article bodies, cisa.gov dynamic paths) — it returns clean markdown but spends metered API credit. Always keep a backup transport; a single failed fetch is never the end of the read.
