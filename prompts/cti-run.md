...
> **Prompt version:** v4.9 — bump in `prompts/CHANGELOG.md` whenever you edit this file. Carry the version through to the run record (`prompt_version` in `runs/<date>/<run-id>.md`). The routine should print this banner at the start of the run so the operator can verify which version executed.
>
...
6b. **Mechanical KEV sweep — every in-window KEV addition gets a disposition (v4.8).** Once `window_hours` is known, run it and keep the output:
   ...
   ```bash
   python3 tools/kev_window_diff.py --window-hours "$WINDOW_HOURS" \
       | tee "work/${RUN_ID}/kev-window.txt"
   ```
