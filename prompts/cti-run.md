...
> **Prompt version:** v4.3 — bump in `prompts/CHANGELOG.md` whenever you edit this file. Carry the version through to the run record (`prompt_version` in `runs/<date>/<run-id>.md`). The routine should print this banner at the start of the run so the operator can verify which version executed.
>
...
<!-- GENERATED from config/org-profile.yaml — do not edit by hand; edit the config and run: python3 tools/compose_prompts.py --write -->
Standing policy / regulatory watch for Swiss federal SOC (Switzerland and Europe, with Swiss and European critical infrastructure and government at the centre · public-sector) — S2 sweeps these every run; a development ships as a `policy` entry only when it changes what the constituency's defenders are obliged or advised to do (PD-11 c):
