...
> **Prompt version:** v4.1 — bump in `prompts/CHANGELOG.md` whenever you edit this file. Carry the version through to the run record (`prompt_version` in `runs/<date>/<run-id>.md`). The routine should print this banner at the start of the run so the operator can verify which version executed.
>
...
2. **Append ONE `updates[]` record** (oldest first; strictly increasing `at`):
   ...
   ```
   and set `updated_at` to the same `at`.
3. **Append the body section** `## <Type> — <at>` — heading exactly `## Update — 2026-…Z` / `## Correction — …` / `## Improvement — …` (the record's `at` verbatim after the em dash) — at the end of the body, carrying **only the inline-cited delta**, never a recap of the entry. Every claim in it follows PD-1/PD-2 like new prose; the deep-read and `grep -F` quote discipline apply.
...
5. **Never touch** the entry id / path, `discovered_at`, `run_id`, `migrated_from`, or any earlier changelog record or section (a wrong earlier update is fixed by a further `correction` record). One record per fire per entry — several changes in one fire are one record.
...
The update is verified exactly like new prose: the Phase 5.7 verifier reads the whole entry, checks the new section and every field named in `fields` against the cited sources, and confirms the record's `summary` states what the section states. The gate FAILs an entry modified without a record for this run (`silent-edit`) and a record without its section (`entry-updates`).
...
- [ ] No candidate duplicating covered ground (incl. earlier runs today, and the store-wide CVE index) shipped as a new entry — every repeat is a changelog record on the existing entry with a material delta, or dropped; every record has its `## <Type> — <at>` section, `updated_at` mirrors it, the frontmatter reflects the current state, and no entry was edited without a record (`silent-edit`).
