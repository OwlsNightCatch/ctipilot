...
> **Prompt version:** v2.69 — bump in `prompts/CHANGELOG.md` whenever you edit this file. Carry the version through to the summary footer (`**Prompt:** vN.M`) and `state/run_log.json.prompt_version`.
>
...
2. **Hard-cap every sub-agent at 30 min wall-clock; do not pre-empt before that.** There is no soft cap below it — depth over speed (see [`.claude/agents/cti-research.md`](../.claude/agents/cti-research.md) § Time-boxing). Past 30 min, abandon and proceed without the sub-agent; log the gap in § 11. Same cap applies to the Phase 4.7 verification sub-agent and to follow-up research sub-agents.
3. **Skeleton-then-Edit.** A single `Write` of the whole file trips `Stream idle timeout`. `Write` skeleton → `Read` → `Edit` per section.
4. **Persist intermediate state often** under `work/<run-id>/` (version-controlled — Phase 5 commits the whole directory alongside the summary).
...
This deployment is parameterized by [`config/org-profile.yaml`](../config/org-profile.yaml). The profile data below is **generated** from that config (`python3 tools/compose_prompts.py --write`; the `compose-profile` GitHub Action keeps it in sync on push) — edit the config, never the generated block. The same profile is composed into `prompts/daily-cti-brief.md`, `.claude/agents/cti-research.md`, and both verifier definitions, so every agent in the workflow sees identical organization context. Empty watchlists and an unconfigured triage scheme are valid — every rule below then no-ops and the summary behaves exactly as it did before this section existed.
...
1. **General landscape first — watchlists never displace it (anti-overshoot).** The weekly's primary mission is unchanged: the week's broader threat picture for the coverage focus above (W-PD-1). Watchlist coverage is a *sharpening lens on top*. Watchlist-driven items are a minority of the summary (guideline: ≤ ⅓ of §§ 1–3 items); when a watchlist item and a general-landscape critical item compete for space, the general critical item wins. A summary that reads like a per-vendor patch report for the watchlisted products is a regression — § 11 must say so when the guideline was exceeded and why.
2. **Relevance boost, not a gate bypass.** A watchlist match (product, supplier, or standing interest) lowers ONLY the relevance bar: an in-window, verified item affecting a watchlisted product/supplier belongs even at moderate severity — but it must still answer one of W-PD-1's three questions and clear every other gate (recency, two-source verification, fake-news guard, link discipline, no IOCs). Never pad: a watchlisted product with no in-window development produces NO item.
3. **Mandatory weekly status sweep — W1 owns it.** W1 runs one consolidated **watchlist status sweep** across the gap window: for each watchlisted product, did the week bring advisories, exploitation-status changes, or KEV/EUVD additions the dailies missed or that accumulated into a cross-day pattern? For each watchlisted supplier, did the week bring breach disclosures, incident reports, or compromise claims (leak-site claims per the fake-news guard)? Batched lookups are the expected shape; results go in W1's findings YAML `watchlist_sweep` block. W2 has no watchlist duty.
4. **Watchlist hits are tagged.** An item included *because of* a watchlist match carries the `watchlist` theme tag in its footer `Tags:`. An item that would have cleared the general bar anyway does NOT carry the tag.
5. **Sweep results are always reported.** § 11 carries one parseable line per run when watchlists are configured: `Watchlist: products checked=N, hits=N; suppliers checked=M, hits=M`. A zero-hit sweep is a healthy outcome. Omit the line when the profile configures no watchlists.

### Org-triage line (static — applies only when the profile defines a triage scheme)

When the generated profile above defines vulnerability-triage categories, every CVE-typed H3 in § 3 (Vulnerability roll-up) ends its body — immediately BEFORE the metadata footer line — with one bold line:
...
Rules: the category is chosen strictly by applying the scheme's criteria to facts the item already cites (exposure class, auth prerequisite, exploitation status, watchlist membership) — the triage line may NOT introduce new facts (the verifier flags drift as F16). If the week changed an item's triage class (e.g. exploitation confirmed mid-week), say so in the clause — that trajectory IS weekly content. If no category's criteria clearly match, use the scheme's default category and say why. When the profile defines no scheme, omit the line everywhere. This is a **body line, not a footer field**: the metadata footer grammar is unchanged.
...
- Network via internal HTTP proxy with allow-list. Hard 30-min per-sub-agent wall-clock cap (anti-crash guard #2).
- Git operations require routine's GitHub App (see `docs/operating.md`). 403 is structural — don't retry.
- **Model is configurable by the runtime.** Self-identify from the harness env vars `CLAUDE_FRIENDLY_NAME` / `CLAUDE_MODEL_ID` when set (see § Self-identification); this prompt deliberately gives no example model name so the unset-env-var fallback isn't biased toward a stale placeholder.
...
intel/<YYYY-MM-DD>/                # closed-source drops (usually absent; Phase 0 step 8 detects, W3 ingests)
work/<run-id>/                     # per-run artefacts — version-controlled, committed in Phase 5
```
...
0. **Capture main-agent start timestamp + compute deterministic run_id (MANDATORY first action).** Before any `Read`, capture an UTC ISO 8601 timestamp and derive a deterministic `run_id`:
   ...
   # (W21 misfire 2026-05-18, W22 misfire 2026-05-25).
...
1. The run targets the **just-completed ISO week** — the week ending on the most recent Sunday — bound to `ISO_WEEK` from step 0 (**not** the wall-clock `date -u +%G-W%V`, which mislabels a fire crossing the Sunday→Monday UTC boundary by one week; that bug shipped the W21/W22 duplicate-weekly misfires). Output filename `briefs/weekly/${ISO_WEEK}.md` (e.g. `2026-W21`). If a file with that name already exists from a previous run **for the same ISO week**, treat as a re-run and overwrite cleanly.
...
1. **Top stories of the week** — by impact, exploitation, home-region/coverage-focus nexus.
...
6. **Items where inaction = incident** (the weekly's defining list). For each item in lists 1–3, ask: *if a SOC reader in the profiled constituency (§ Organization profile) did not act on this when it appeared in the daily, would they currently be in an incident?* Inputs that move an item onto this list:
...
4. **Source-list slice** — tiered like the daily's Phase 0 allocation: all matching `tier: essential` records first, then the stalest matching `tier: standard` records, passed inline with each record's `id`, `publisher`, `url`, `rss_url`, `tier`, `fetch_method`, `reliability`, `language`, and newest dated recipe note — the record is the recipe. (The every-run essential-coverage *guarantee* is the daily routine's; the weekly applies the same prioritisation without the coverage check.)
...
6. **Rotation-priority list** — the standard-tier records in this slice flagged as gaps in 2+ daily briefs in the window, named explicitly so the sub-agent works them right after the essential records.
...
### Conditional W3 — closed-source intake (spawned only when intel files exist)
...
Search for cybersecurity-policy developments relevant to the profiled constituency from the gap-derived window, driven by the standing policy watch below (`policy_watch` in config/org-profile.yaml). The national-CERT carve-out applies for primary disclosures.

<!-- ORG-PROFILE:BEGIN org-policy-watch -->
<!-- GENERATED from config/org-profile.yaml — do not edit by hand; edit the config and run: python3 tools/compose_prompts.py --write -->
Standing policy / regulatory watch for Swiss federal SOC (Switzerland and Europe · public-sector):
...
### Sub-agent return format (findings on disk + compact summary — same contract as the daily)

The `cti-research` definition owns the return contract (findings-on-disk shape — see [`.claude/agents/cti-research.md`](../.claude/agents/cti-research.md) § Return format): each sub-agent writes `work/<run-id>/findings.<W1|W2>.yaml` (structured items with `sources`, `discovery_trace`, `summary`, `evidence` quotes, `verification`, `confidence`, `novelty`, plus the optional `watchlist_sweep` block) and returns only the ~150-token compact summary (`**Model:**` / `**Timestamps:**` / `**Findings:**` / `**Candidate sources:**` / `**Coverage gaps:**` lines). The weekly-specific `novelty` values are `new | update-to-prior:weekly:YYYY-Www | duplicate-of-daily:YYYY-MM-DD`. The legacy free-form Markdown return is the documented fallback only when the YAML write fails.

If a sub-agent finds nothing it writes an empty `items: []` YAML with a one-line note and returns `**Findings:** 0 items`. Empty weeks on the horizon axes are valid.
...
7. **Rank** within each target section by exploitation > home-region/coverage-focus nexus > primary-sector nexus > cross-day-pattern strength > horizon significance.
...
### Compose strictly from the findings files (anti-embellishment, mirrors the daily)

Verifier telemetry across past runs shows the two dominant defect classes are **F3 (claim-not-supported)** and **F4 (hallucinated-fact)** — composition-time embellishment, not research failure. The weekly rule mirrors the daily's Phase 4 § Compose strictly from the findings files:

1. **Every factual claim in every item must trace to (a) the item's record in `work/<run-id>/findings.<W1|W2>.yaml`, (b) a daily brief in the gap window (itself source-backed — follow the chain and cite the original source), or (c) a page you yourself spot-checked in Phase 2.5.** No enrichment from memory; no inferred connections between items (F13). If a detail feels missing, spawn a scoped follow-up research sub-agent or leave it out.
2. **Carry the source's technical phrasing; tighten, never escalate.** Synthesis across the week is the weekly's job — upgrading claims while synthesising is not (F14).
3. **Numbers, counts, and superlatives come only from `evidence` quotes, `summary` text, or a daily's cited source.**
4. **Evidence escalation.** The `Evidence:` footer field is REQUIRED on every § 1 item (§ 1 is the weekly's highest-trust section) AND on every § 3 item whose footer `Status:` includes `exploited`. Populate it verbatim from the findings YAML's `evidence` records or the originating daily's `Evidence:` field — never write a quote neither carries. `tools/check_brief.py` WARNs (`evidence-presence`) on exploited-status items missing the field — fix those WARNs before Phase 4.7.
...
**Source-link discipline (numbered).** (1) Only fetched URLs — every URL must have been opened by `WebFetch` or `tools/fetch_source.py` in this run, resolving to content matching the claim. Never construct a URL from a pattern (advisory ID, CVE ID, blog-slug guess) without verifying. (2) Specific page, never the landing — see hard-blocked patterns above. (3) Drill to primary, keep secondaries — first link most primary; include every other URL where you read the claim as `· Additional source:`. (4) News-only fallback acceptable when explicit (cite specific article URL, never homepage; flag in § 11). (5) Verify before publishing — re-confirm doubt cases; if a URL 404s, redirects to homepage, or shows unrelated content, replace or drop. (6) If unsure: drop the item.
...
A single `Write` of the whole 12-section file trips `Stream idle timeout — partial response received`. **Required pattern:**
...
**§ 3 Vulnerability roll-up.** Per-CVE H3 entries for the **operationally critical** CVEs of the week (Active ITW, KEV-added during window, pre-auth RCE on internet-exposed software, supply-chain compromise affecting widely-deployed software), each with status-this-week-vs-first-covered prose and a per-item footer. **No CVE summary table** — the per-item footers already carry CVE / CVSS / Vector / Auth / Status, and a summary table is a recurring source of patched-version / KEV transcription errors. A CVE that cleared the daily's § 3 gates but is now patched with no exploitation evidence can be folded into a one-line mention inside a related H3 rather than getting its own. Use per-CVE breakdown notation in the footer when an H3 covers more than one CVE.

**§ 4 Sector & victim patterns.** One H3 per sector that saw meaningful activity in the window. Where a sector inside the org's home region / coverage focus (§ Organization profile) saw activity, lead with that. Avoid generic sector commentary — every claim needs an inline source link to a specific incident or report.
...
**§ 9 Policy & regulatory horizon.** Sub-agent W2 output. Items that change the profiled organization's SOC obligations directly — developments across the standing policy watch (the org-policy-watch list under W2) and its regulators. Each item explains *what changed* and *what defenders need to do differently*.

**§ 10 Looking ahead — what to watch next week.** A focused, justified list. **Not predictions** — items already in motion that are likely to develop next week (vendor advisories with patches mid-rollout, campaigns still acquiring victims, regulatory consultations closing, policy-watch regulator deadlines approaching, ongoing exploitation against named target classes). Each item links back to the relevant earlier reporting. No footer per item; this is a list section. Per the inherited PD-13, **a pending CISA KEV remediation deadline is not on its own a Looking-ahead item** — it's a US-FCEB compliance date; the underlying exploitation trajectory is what to surface.
...
Append a per-run record. **`run_id` is mandatory and idempotent:** the deterministic id you computed in Phase 0 step 0 (`<YYYY-Www>-<sha8 of brief_path|started_minute>`). Before appending, scan `runs[].run_id` — if an entry with this `run_id` already exists, **do not append a duplicate**; instead, update the existing record in place (this makes a Phase-5-retry safe). The `tools/check_brief.py` `run-log-fields` check FAILs on a missing or duplicate `run_id`.
    ...
    "iterations": [
      {
        ...
        "findings": [                                        // RICH per-finding records (REQUIRED every iteration; [] on CLEAN)
          {
            "code": "F1",                                    // F1..F16
...
7. Every H3 in §§ 1–9 ends with a metadata footer.
...
- **Editorial-quality gate** — every item assessed for relevance to the profiled organization's SOC (§ Organization profile lens), primary-source strength, signal-to-noise, vendor-marketing tells, missed angles, **and weekly-specific framing**: does each item answer one of W-PD-1's three questions (inaction = incident / cross-day pattern / strategic horizon)? Items that don't are flagged for drop or re-framing.
...
Both agent definitions ([`.claude/agents/cti-verification.md`](../.claude/agents/cti-verification.md), [`.claude/agents/cti-verification-alt.md`](../.claude/agents/cti-verification-alt.md)) carry the **identical operational system prompt** — gatekeeper framing + anti-hallucinated-findings clause, truth checks 1–4 (URL fetched, lands on specific article, supports the claim, named entities cross-checked), editorial-quality checks 5–10, whole-brief checks 11–13 (including the W-PD-1 weekly question: does each item answer one of *inaction = incident* / *cross-day pattern* / *strategic horizon*), the composed organization context (relevance + watchlist-tag + org-triage awareness), return format with finding categories F1–F16 (F7 covers the weekly-specific drop case for pure one-to-one daily summaries; F12 is single-source missing-flag; F13–F15 are truth-class — analytical-link-as-fact, quantifier-without-source, name-collision-unflagged; F16 is org-triage, editorial), verdict line, mandatory `**Model:**` + `**Timestamps:**` self-identification, 30-min hard runtime cap. The only difference is the model frontmatter pins.
...
Track verification iterations in the run log: `state/run_log.json` fields `verification_iterations`, `verification_residual_count`. The Ops dashboard reads these. **`verification_residual_count` semantics:** `0` when the final iteration's verdict is `CLEAN`; `(final_iter.truth + final_iter.editorial)` when the final iteration's verdict is `NEEDS_FIXES` (cap reached). Advisory (F11) excluded — F11 alone never blocks CLEAN. **Never `0` on a NEEDS_FIXES final iteration.** `tools/check_brief.py`'s `cap-breach` WARN reads this and surfaces to the Ops dashboard.
...
**1. Stage and commit.** **Commit the per-run `work/<run-id>/` directory alongside the weekly** — same rationale as the daily routine: sub-agent findings YAMLs, verification iteration reports, the URL-liveness ledger, per-agent timestamp checkpoints, the prior-coverage and state-summary snapshots are all needed for post-publish forensics. See [`prompts/daily-cti-brief.md`](daily-cti-brief.md) § Phase 6 for the rationale paragraph.
...
- [ ] § 1 leads with items where active exploitation, missed deadlines, or campaign continuation make inaction = incident — or explicitly states the section is empty for the week. **Every § 1 item carries an `Evidence:` field** binding its load-bearing exploitation claims to fetched-source quotes; so does every § 3 item whose `Status:` includes `exploited`.
...
- [ ] Every H3 item in §§ 1–9 ends with a metadata footer using only taxonomy values.
