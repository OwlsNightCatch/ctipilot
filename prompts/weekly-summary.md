...
> **Prompt version:** v2.68 — bump in `prompts/CHANGELOG.md` whenever you edit this file. Carry the version through to the summary footer (`**Prompt:** vN.M`) and `state/run_log.json.prompt_version`.
>
...
6. **Items where inaction = incident** (the weekly's defining list). For each item in lists 1–3, ask: *if a Swiss / EU public-sector SOC reader did not act on this when it appeared in the daily, would they currently be in an incident?* Inputs that move an item onto this list:
