...
| `legacy-shape` | An entry carries a deleted field (`horizon`, `weekly_section`) or a kind outside `content_model.ACTIVE_KINDS`, or the run record's kind is `weekly` | The weekly routine and its entries are gone: drop the field, use a current kind; run kinds are `intel` / `audit` |
