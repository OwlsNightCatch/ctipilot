...
## 0. TL;DR
- {bullet with inline source link} (3–6 bullets)
...
*(Exploited-status § 1 / § 2 items require the `Evidence:` field. An item included because of an org-profile watchlist match additionally carries the `watchlist` tag in `Tags:`.)*
...
**Org triage ({short_name}):** {category-id} — {category name}. {One clause; note when the week changed the item's triage class.} *(Only when `config/org-profile.yaml` defines a triage scheme — omit otherwise. Exploited-status § 3 entries also require the `Evidence:` footer field.)*
