...
> **Prompt version:** v2.69 — bump in `prompts/CHANGELOG.md` whenever you edit this file. Carry the version through to the brief footer (`**Prompt:** vN.M`) and to `state/run_log.json.prompt_version`. The routine should print this banner at the start of the run so the operator can verify which version executed.
>
...
2. **Hard-cap every sub-agent at 30 min wall-clock; do not pre-empt before that.** There is no soft cap below it — depth over speed (see [`.claude/agents/cti-research.md`](../.claude/agents/cti-research.md) § Time-boxing for the rationale: deep pivots, no fixed fetch budget). Past 30 min, abandon and proceed without the sub-agent; log the gap in § 7. Same cap applies to the Phase 5.7 verification sub-agent and to follow-up research sub-agents spawned during verification iterations.
...
4. **Persist intermediate state often** under `work/<run-id>/<step>.json` (version-controlled — Phase 6 commits the whole directory alongside the brief). After every meaningful unit of work — every fetched source summarised, every CVE enriched, every section drafted — write the partial result so a later step can resume.
...
9. **Main agent does NO source fetching during Phase 1 (anti-classifier-trip).** While the four `cti-research` sub-agents are running, the main agent MUST NOT call `WebFetch`, `WebSearch`, or `python3 tools/fetch_source.py`. Source-fetching is the sub-agents' exclusive job in Phase 1; their isolated contexts absorb the raw advisory / breach / enforcement content so the main agent's working context stays compositional rather than research-loaded. Two failure modes the rule prevents: (a) **duplicate work** — fetches the sub-agent was already going to do, wasted wall-clock and rate-limit budget; (b) **classifier trip** — accumulating raw CTI content (NCSC.ch / ICO / ANSSI / ENISA advisory bodies, breach-enforcement listings, exploit-detail vendor PSIRTs) on top of the 25 K-token prompt baseline + the 30 K-token Phase 0 digests has tripped Anthropic's "violative cyber content" classifier on past runs, killing the brief mid-Phase-2 with `API Error … Usage Policy` and **no published file** (the worst PD-1 violation). The only main-agent invocations of those tools are: Phase 2 verification (per-item URL spot-checks, one or two URLs at most), Phase 5.7 verification-fix iterations (re-fetching a single primary to replace a broken or generic URL the verifier flagged), and Phase 7 publish polling (`curl` on the site index — not a CTI fetch). Anything else: spawn another sub-agent. **The record-driven fetch methodology lives in [`.claude/agents/cti-research.md`](../.claude/agents/cti-research.md) § Working the source list + § Fetch tooling reference** — the sub-agents read it; the main agent does not. Hardened as META hard invariant #16.
...
11. **Less is more — relevance over volume.** Every item costs reader attention. Ship fewer, sharper items. An item belongs only if ≥1 is true: (a) changes what a SOC in the profiled constituency (§ Organization profile) patches, hunts for, blocks, or detects in 1–7 days; (b) freshly-disclosed actively-exploited vulnerability or campaign with concrete defender-actionable specifics (component, prerequisite, detection/mitigation step); (c) confirmed home-region / primary-sector incident, regulatory action, or victim disclosure with operational lessons (root cause, kill-chain, segmentation gap, identity weakness); (d) substantive primary technical analysis materially improving understanding of an attack technique.
...
13. **CISA KEV remediation deadlines are not operational signal for this audience.** The profiled audience (§ Organization profile) is not a US federal civilian (FCEB) agency. CISA's Known Exploited Vulnerabilities catalog is referenced for **one** thing: the **listing flag**, which is CISA's confirmation that a CVE is being actively exploited in the wild — that signal is jurisdiction-agnostic intelligence and stays useful (record `Status: ..., cisa-kev` in the footer when applicable, leave the `cisa-kev` taxonomy tag in place). The **remediation deadline** attached to a KEV entry is a US Federal Civilian Executive Branch (FCEB) compliance date under [BOD 22-01](https://www.cisa.gov/news-events/directives/binding-operational-directive-22-01); it has **no jurisdictional weight for the profiled constituency** and **never** justifies brief real estate on its own. Concretely:
    ...
    - **Same logic** applies to other deadlines issued to agencies outside the profiled jurisdiction (CISA Emergency Directives, US ED orders) — note them as context if relevant, never as the operational driver.
...
4. **Watchlist hits are tagged.** An item included *because of* a watchlist match carries the `watchlist` theme tag in its footer `Tags:` so readers and the trends dashboard can slice org-specific signal. An item that would have cleared the general bar anyway does NOT carry the tag.
...
intel/<YYYY-MM-DD>/                # closed-source drops (usually absent; Phase 0 step 8 detects, S5 ingests)
...
1. **Generate the structured H3-record + state digests via scripts (MANDATORY — token-budget guard).** The main agent must NOT `Read` every brief from the last 7 days into its own context — at 50–80 KB per brief, seven dailies plus the latest weekly run the main agent ~120 K tokens of input *before Phase 1 starts*. Instead, build the two compact summaries via Bash and `Read` only the keys-only digest in the main agent's context:
   ...
   Token-budget reduction: the full-Read approach costs ~120 K tokens before Phase 1; the script-built digests cut that to ~30 K; keeping only the keys index in the main agent's context cuts it to ~21 K. **The full state files and brief bodies are still on disk** — sub-agents `Read` them directly when needed; the main agent on-demand-`Read`s a specific daily / weekly body only when Phase 4 composition needs to quote one.

2. **`Read work/${RUN_ID}/prior_coverage_keys.json`** (keys-only). Dedup index for the main agent — `{key, date, brief_path, section}` per H3 with **no titles, no one-line tl;drs, no primary-source URLs**. That's all the main agent needs to answer "is this candidate's key already covered? yes/no" during Phase 2 dedup and to drive §-1-vs-§-2-vs-§-3 / UPDATE-vs-new split decisions in Phase 4. Sub-agents read the full `prior_coverage.json` (with prose and URLs) in their isolated contexts for fetch-time PD-8 dedup — see the spawn-message contract in Phase 1. When you need a specific item's full record for Phase 4 composition (e.g. composing a § 4 UPDATE that references the prior coverage's primary URL), `jq` the full file on-demand:

   ```bash
   jq '.records[] | select(.key == "<key>")' "work/${RUN_ID}/prior_coverage.json"
   ```

   Keeping the prose out of the main agent's working context cuts the dense-CTI baseline (CVE titles like "pre-auth RCE … KEV deadline …", incident summaries listing victims and exploitation patterns) by ~8 K tokens and reduces cyber-content classifier risk on top of the prompt's own vocabulary load.
...
1. **Essential floor — non-negotiable, every daily run.** Every `sources.json` record with `status: active` AND `tier: essential` (national CERT / NCSC / CISA / ENISA-class authorities + exploitation ground truth) goes into the slice of the sub-agent whose category filter matches it (a record matching several domains goes to the first-listed matching domain — never to two agents). **ALL essential sources are attempted every daily run**; a miss is disclosed in § 7 (`Essential-coverage:` line) and flagged by `check_brief.py` (`essential-coverage`).
2. **Staleness rotation for `tier: standard`.** Rank each domain's matching standard records oldest-`last_successful_fetch` first (never-fetched before everything), promote `state-summary.json.runs.fetch_gaps_in_window` entries to the top, and take roughly the top 10–14 per agent. This is the balance mechanism: no single run floods an agent with the whole list, and no source silently starves — every standard source is attempted at least every ~7 runs. Standard sources NOT allocated today are simply tomorrow's freshest rotation candidates; nothing is dropped, only scheduled.
3. **Mark the tier on every record in the slice** so the sub-agent knows what is mandatory (essential) versus rotational, and can prioritise accordingly (its § Working the source list covers the semantics).
...
1. **Run identifier** — `Run id: <YYYY-MM-DD>-<sha8>` (the deterministic run_id from Phase 0 step 0) so the sub-agent knows which `work/<run-id>/` directory to checkpoint into. Pre-create the directory before spawning.
2. **Recency window** — `window_hours: <N>` from Phase 0 step 7.
3. **Domain** — one of S1 / S2 / S3 / S4 with the source-filter table below.
4. **Source-list slice** — the tiered allocation from Phase 0 (ALL matching `tier: essential` records + the staleness-rotation subset of `tier: standard` records), passed inline with each record's `id`, `publisher`, `url`, `rss_url`, `tier`, `fetch_method`, `reliability`, `language`, and the newest dated recipe note — the record is the recipe; the sub-agent is never told which publishers matter beyond what the slice itself says.
5. **Dedup context** — CVE IDs from `cves_seen.json`, named entities from `covered_items.json`, headlines / first paragraphs of last-7-days briefs, the most recent weekly's top stories. **Plus** `prior_coverage_records: <count>` and the path `work/<run-id>/prior_coverage.json` — the structured per-H3 records (key, title, one-line tl;dr, primary-source URL, date) for every item in the last 7 daily briefs. The file is generated by Phase 0 step 1; sub-agents read it before fetching so they can dedup against full records, not just headlines. **PD-8 enforcement at fetch time, not just at main-agent Phase 2 dedup time.**
6. **Rotation-priority list** — the standard-tier records in this slice that came from `fetch_gaps_in_window` (missed on 2+ recent runs), named explicitly so the sub-agent works them right after the essential floor.
...
### Reinforced rules — for the sub-agents during fetch, and for the main agent during Phase 2 spot-checks + Phase 5.7 re-fetches

**These rules belong to the `cti-research` sub-agent's operational system prompt — they govern fetching.** The record-driven fetch methodology (each source record carries its own recipe), the `WebFetch` outbound-links template, the structured-endpoint subcommand catalog, and the discovery-trace shape all live in [`.claude/agents/cti-research.md`](../.claude/agents/cti-research.md) (§ Working the source list, § Fetch tooling reference, § `WebFetch`, § Source-link discipline, § Discovery trace). **The main agent does not read or follow the fetch methodology** — it does no source fetching during Phase 1 (anti-crash guard #9), and during Phase 2 / Phase 5.7 single-URL spot-checks the main agent calls `WebFetch` only on a per-article URL that's already in a sub-agent's `Sources:` block (the sub-agent already navigated the bridge). The main agent's only direct bridge call would be a Phase 5.7 verification-fix re-fetch when the verifier flagged a broken URL and you need to re-pivot — that one case uses `python3 tools/fetch_source.py url <new specific URL>`, never the listing-page subcommands.

The two rules the main agent DOES apply when consolidating sub-agent returns and composing the brief (these are about citation discipline, not fetching):

1. **Drill into curated sources at citation time too.** If a sub-agent returned a homepage / listing / category-landing URL as a Source (it should not have, but happens), drop the item to § 7 rather than promoting the generic URL. Phase 5.5's `tools/check_brief.py` URL allowlist FAILs the commit on those patterns anyway — catch it before the script does.
2. **Source-link discipline at composition time.** First link in every footer is the most primary the sub-agent returned (vendor PSIRT > vendor research blog > research-lab post > regulator filing > victim disclosure > national CERT/CSIRT > MITRE/NVD > ENISA EUVD > news). Include every other URL the sub-agent surfaced as `· Additional source:` rather than dropping them — they are corroboration. If unsure about a URL, drop the item to § 7.

Phase 5.5 enforces both rules mechanically: `tools/check_brief.py` runs the URL allowlist on every footer's `Source:` and live-HEAD/GETs every URL; an item whose Source is a homepage/listing/category-landing or whose URL 404s FAILs the commit. The main agent's job here is to not let the sub-agent's bad URLs reach the commit, not to re-do the bridge work.
...
### Conditional S5 — closed-source intake (spawned only when intel files exist)
...
**Trigger:** as soon as **all returning sub-agents have returned**. Sub-agents stalled past the 30-min hard cap are abandoned. Do **not** wait indefinitely.
...
7. **Rank** by exploitation > home-region/coverage-focus nexus > primary-sector nexus > novelty.
...
1. Active in-the-wild exploitation **and** non-trivial exposure for the profiled constituency (§ Organization profile).
...
```bash
AGENTS="S1 S2 S3 S4"
[ -f "work/${RUN_ID}/S5.started_at" ] && AGENTS="$AGENTS S5"   # include S5 when Phase 0 spawned it
for k in $AGENTS; do
...
Verifier telemetry across past runs shows the two dominant defect classes are **F3 (claim-not-supported)** and **F4 (hallucinated-fact)** — together roughly half of all findings, and they enter at composition time: the research was sound, the brief's prose then went beyond it. Each such finding costs a verification iteration; most runs burned 4–5 iterations. The remedy is mechanical:
...
Numbering is **dense and stable** — never skip a section number. Immediate Actions is *not* its own H2; on most days no item clears the bar, and on the rare day one does it appears as a **callout block inside § 0 TL;DR, immediately after the bullet list** (see § 0 below). § 4 Updates sits **above** § 5 Deep Dive intentionally so daily readers following an ongoing story see the new development before hitting long-form.

The org-lens emphasis (home region / coverage focus / sectors — § Organization profile) is per-item **region/sector tags** in § 1 — order § 1 org-lens items first, then global, then rest (no separate home-region section).
...
**`Evidence:` field (source-quote binding).** When the sub-agent who researched the item extracted verbatim quotes from the fetched sources, the item's footer carries an `Evidence:` field listing those quotes with attribution. Shape:
...
CVEs that don't clear a gate **stay out** — log dropped CVEs in § 7 with reason. **No CVE summary table** — the per-item footers already carry CVE / CVSS / Vector / Auth / Status, and a summary table is a recurring transcription-error source. S1's findings YAML `cve_table:` records are structured input for the per-CVE H3 entries and the state updates, not a rendered table.
...
**The full taxonomy of what each of those means** — the prescriptive list of MITRE T-IDs, Sysmon / Windows / `auditd` event IDs, identity-protocol-abuse names (NTLM relay, OAuth device-code, SAML response forgery, S4U2Self), named-cluster patterns (UNC####, Storm-####, TA####, APT##, CL-###-####), affected-version phrasing — **lives in [`.claude/agents/cti-research.md`](../.claude/agents/cti-research.md) § Technical depth**. The sub-agents apply that vocabulary at research time and surface it verbatim in their returns; the main agent's job is to faithfully carry the sub-agent's specificity into the brief, not to invent new technical detail on top.
...
- **Metadata drift correction** — a fetch also re-validates the source's recorded metadata, not just its counters. When what the fetch returns no longer matches the record, **correct the field in place** (append-only dated note): (a) `fetch_method` → the recipe that actually worked this run (the documented WebFetch 403s but the RSS feed serves cleanly → `rss`; the landing page is an empty SPA shell but a bridge subcommand works → `bridge`/`api`); (b) `category` → add/remove tags to match what the source actually publishes (a research lab that began shipping CVE write-ups gains `vulns`; a vendor lab mis-tagged `gov` loses it), values from the controlled vocabulary only; (c) `reliability` → raise or lower one tier when sourcing quality demonstrably changed. This is the soft-removal/soft-promotion of *quality* metadata that complements the status axis above. **Every such edit (and every status transition, new candidate, and url change) MUST be recorded in `state/run_log.json.sources_changed[]`** (schema below) so the Ops dashboard highlights what moved this run.
...
```jsonc
{
  ...
  "tier": "standard",                    // essential | standard. Candidates start standard; `essential` is reserved for national-CERT/NCSC/CISA/ENISA-class authorities + exploitation ground truth (queried EVERY daily run) and is normally an operator decision — record any tier change in sources_changed[].
...
- Every active or demoted source MUST carry `publisher`, `reliability`, `tier`, `language`, `fetch_method`, and `consecutive_failures` — the shape check FAILs on a partial entry promoted out of candidate.
...
```jsonc
{
  ...
  "fetch_failures": [                                         // RICH SHAPE required; legacy `{id, code}` still parses but the dashboard renders it as a yellow "needs-detail" row
    {
  ...
  "sources_changed": [                                        // every sources/sources.json edit this run; the Ops dashboard "Sources changed" panel + runs-table "Sources Δ" column read this. `[]` on a run that only refreshed last_successful_fetch / counters.
    {
      "id": "github-advisory",                                // source id touched (REQUIRED)
      "change": "promoted",                                   // promoted | demoted | added | recategorised | reliability | fetch_method | url | recovered | tier (REQUIRED)
  ...
  "verification": {                                           // per-iteration breakdown
    "iterations": [
      {
        "n": 1,
      ...
      /* one entry per iteration, up to 5 */
    ]
  }
}
```
...
- `returned: false` only when stalled past the 30-min hard cap (renders as `stalled` badge).
- `fetch_failures` = ONLY real, unrecovered failures — sources the brief needed where neither the direct URL, the documented bridge recipe, nor any documented fallback (a corroborating alternate publisher) returned usable content. **Every record must end in `covered_anyway: false`** (a record with `covered_anyway: true` is a soft signal — see § fetch_failures reporting in `.claude/agents/cti-research.md`; the dashboard tags any such record yellow). Every entry must carry `id`, `url_tried` (verbatim), `fetch_method`, `status_code`, `error_class`, `error_message`, `attempted_methods` (ordered), `mitigation_applied`, `covered_anyway`. **Do NOT log:** "bridge fetched OK; no new in-window content" (that is success, not failure — quiet days are not gaps); WebFetch 403 on a known-403 host where the bridge then succeeded (that is the documented recipe working as intended); SPA-empty landing pages where a structured-endpoint bridge subcommand worked; sources where `covered_anyway: true` via a deterministic alternate (the story reached the brief). The Ops dashboard's "Coverage gaps (latest run)" panel reads this field; every row is, by definition, a real gap that the operator should look at.
- `sources_changed` = one entry for every edit you made to `sources/sources.json` this run: each status transition (candidate→active, →demoted, recovered), each new `candidate` (`change: "added"`, `from: ""`), and each metadata-drift correction (`change ∈ recategorised | reliability | fetch_method | url`). `[]` when the run only refreshed `last_successful_fetch` / counters. Each entry carries `id`, `change`, `from`, `to`, `reason`. The Ops dashboard `/ops/` highlights these per run; an empty list renders as `—`. This is the operator's at-a-glance view of how the source list evolved on each fire.
- `bridge_uses` = optional record of bridge-subcommand invocations even when they succeeded (telemetry for "is the bridge doing its job"). Sub-agents include a `## Bridge uses` section in their return; the main agent counts entries into `state/run_log.json.bridge_uses[]` with `{id, method, outcome ∈ ok | empty-feed | item-not-found}`. This keeps "successful bridge fetches that found no content" out of `fetch_failures[]`, where they would read as gaps. Optional; missing section means no bridge-use telemetry, dashboard shows `—`.
...
### `state/source_health.json` — periodic accessibility probe (run at the END of every run)
...
- **Footers (FAIL)**: every H3 in `immediate-actions / active-threats / trending-vulnerabilities / research / updates / deep-dive / action-items` ends with a metadata footer; Source (≥1 link), Tags, Region required; CVE-typed entries also carry CVE / Vector / Auth / Status; multi-CVE items use single shared CVSS or per-CVE breakdown; every Tag / Region / Sector / Vector / Auth / Status value is in `site/taxonomy.yaml`.
...
- **Editorial-quality gate** — every item assessed for relevance to the profiled organization's SOC (§ Organization profile lens), primary-source strength, signal-to-noise, vendor-marketing tells, missed angles. Off-audience items flagged for drop.
...
1. Receive report. **CLEAN → proceed to Phase 6.** NEEDS_FIXES → apply remediation per finding type:
    ...
    | Analytical-link-as-fact (F13) | Soften or drop the asserted connection. If a source genuinely supports the link, re-cite that source on the connection claim and rewrite the sentence so the link is presented as the source's claim, not the brief's analytical inference (e.g. "echoes the credential-leakage pattern" rather than "the same mechanism TeamPCP used"). |
    | Quantifier without source (F14) | Replace the quantifier with the value the source actually states, or drop it. "Five unpatched zero-days" → either "four" if the source enumerates four, or "several" if the source doesn't count, or omit if the count was load-bearing on a non-counted source. |
    | Name-collision unflagged (F15) | Add an explicit disambiguation phrase to the body ("named for the attacker tooling", "no relation to the X campaign", "not to be confused with"). If the H3 is actually an update to the prior coverage, restructure it as an `UPDATE:` block linking back. The Datadog Shai-Hulud inversion would have been remediated by either rewriting the §4 UPDATE to correctly describe Datadog *analysing* the leaked attacker code (which is what the source says) or, if Datadog had released a genuine same-name defender tool, by adding "named for the attacker worm Datadog Security Labs previously analysed." |
    | Org-triage missing / inconsistent (F16) | Add or correct the `**Org triage ({short_name}):**` line per § Org-triage line policy: choose the category strictly by applying the generated scheme's criteria to facts the item already cites; if the triage clause referenced an uncited fact, rewrite it to reference only cited facts (or add the citation). When the profile defines no scheme, remove the line entirely. |
...
- **Capture the verifier's model, timestamps, AND the actual findings on every iteration.** The verification sub-agent's return opens with `**Model:** <friendly name> (`<model-id>`)` followed by `**Timestamps:** started_at=… · ended_at=… · duration_seconds=…`, then the `### F1 …` / `### F2 …` numbered finding sections. Append a record to `state/run_log.json.verification.iterations[]` for every iteration:
  ...
  ```jsonc
  {
    ...
    "findings": [                                            // RICH per-finding records (REQUIRED on every iteration)
      {
        "code": "F1",                                        // F1..F16
  ...
  The Ops dashboard renders one row per iteration with the verifier model, duration, and **per-finding details for the final iteration** (the cap-breach signal). Missing `**Model:**` line → `"unknown"`. Missing `**Timestamps:**` line → `"unknown"` for both timestamps and `null` for `duration_seconds`. Missing `findings[]` (legacy records) → empty array; the dashboard renders the legacy `truth/editorial/advisory` scalars in that case but flags the iteration with a yellow "no per-finding detail recorded" badge.
- Track in `state/run_log.json`: `verification_iterations`, `verification_residual_count`, **`verification.iterations[]`** (per-iteration breakdown; the legacy two scalar fields stay for back-compat with older briefs). **`verification_residual_count` semantics:** when the **final** iteration's verdict is `CLEAN`, `verification_residual_count = 0`. When the final iteration's verdict is `NEEDS_FIXES` (cap reached without CLEAN), `verification_residual_count = (final_iter.truth + final_iter.editorial)` — F11 advisory excluded because F11 alone never blocks CLEAN. **Counting it 0 on a NEEDS_FIXES final iteration silently absorbs an editorial-quality drift the gatekeeper was supposed to catch — that mistake is what the cap-breach yellow signal exists to prevent.** The `tools/check_brief.py` `cap-breach` WARN reads this field and surfaces the cap-breach to the operator's Ops dashboard.
...
**1. Stage and commit on the current branch.** **Commit the per-run `work/<run-id>/` directory alongside the brief** so sub-agent findings (`findings.<S1|S2|S3|S4>.yaml`), verification reports (`verification.iter<N>.md` / `.findings.yaml`), the URL-liveness ledger (`url-liveness.tsv`), per-agent timestamp checkpoints (`<domain>.started_at` / `.ended_at`), the prior-coverage snapshot (`prior_coverage.json` + `prior_coverage_keys.json`), and the state digest (`state-summary.json`) are auditable in git history. The directory is the operator's primary forensic surface when a brief surfaces a defect post-publish — without it, the question "which sub-agent fetched which URL with which evidence quote?" has no answer.
...
brief_path="briefs/$(date -u +%F).md"
DEADLINE=$(($(date +%s) + 600))
# The site URL is deployment-configurable (empty = private deployment
...
- [ ] **Essential-source coverage** — every active `tier: essential` source appears in some sub-agent's `sources_attempted` this run, or § 7 carries the `Essential-coverage: missed=…` disclosure.
