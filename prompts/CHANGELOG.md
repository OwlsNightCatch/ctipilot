...
Tracks substantive changes to `prompts/cti-run.md` (before v3.0: `prompts/daily-cti-brief.md`) and `prompts/quality-audit.md` — and, until its retirement in v4.0, `prompts/weekly-summary.md`.
...
Operator directive, 2026-08-27, landed on top of v4.0 the same day. The routine moves to the Claude 5 family: intel fires on Claude Sonnet 5, the quality audit on Claude Opus 5, and every sub-agent on Sonnet 5. Three things followed from that.

1. **The Opus verifier pin and the two-definition rotation no longer describe anything.** `cti-verification` pinned `opus` and `cti-verification-alt` pinned `sonnet` so that consecutive Phase 5.7 iterations ran on two different models; the operator's instruction is that verification runs on Sonnet 5 only. With one model there is nothing to rotate, and keeping two byte-identical files that must be regenerated in lockstep buys nothing but a maintenance rule. The alt definition also carried the pipeline's worst spawn-failure history (2026-07-23, 2026-08-06: every alt spawn classifier-blocked, whole fires forced onto one model, the v3.31 model-override ladder invented to recover it) — machinery that existed only because there were two definitions.
2. **The publish gate's value was the second independent pass, not the second model.** The audits' own evidence from the rotation era (2026-07-26, 2026-08-02, 2026-08-09) is that confirmation passes catch remediation regressions and fresh-eyes defects — findings that come from reading cold, not from a different model's blind spots. The operator keeps the two-pass requirement and drops the two-model one.
3. **The Series 5 models read prompts differently.** Sonnet 5 follows instructions literally and at the scope stated, calibrates length to the task, and — the documented code-review harness effect — under-reports when a review prompt says "be conservative" or "do not pad", converting the same depth of investigation into fewer reported findings. Opus 5 verifies its own work unprompted, narrates readily, writes longer documents, and over-verifies when a prompt adds re-check scaffolding. Prompts tuned for the previous generation needed those behaviours addressed explicitly.

### What changed

- **`.claude/agents/cti-verification.md`** — rewritten as the single verifier definition: `model: claude-sonnet-5` (explicit id, not the alias), `effort: xhigh` (the pipeline's hardest agentic task; previously Opus at `high`). Rotation, alt-definition and model-override language removed. The gatekeeper section states the Sonnet 5 finding-stage contract directly: report every defect you can evidence, low-severity and uncertain ones included, marked `(low confidence)` where you cannot fully confirm — the bar is evidence, not severity, and the main agent filters downstream. The prior-iteration deltas block is attached to every iteration that follows a NEEDS_FIXES (not to even iterations); a confirmation pass after a CLEAN reads cold. v4.0's additions are kept verbatim — the updated-entry scope, truth check 4c (the changelog contract), the F7 duplicate and F15 changelog wording, the quality-audit coverage-shape bullet. F1–F18, the disk-report structure, the findings YAML, the compact return lines, the timestamps files and the 30-min cap are unchanged so the main agent, `check_run.py` and `site/build.py` parse exactly what they parsed before. Findings are told to be terse — quoted claim, source line, one sentence on the gap.
- **`.claude/agents/cti-verification-alt.md`** — deleted. `tools/compose_prompts.py` TARGETS, `.github/workflows/compose-profile.yml` (path trigger + `git add` list) and the `config/org-profile.yaml` header comment no longer list it.
- **`.claude/agents/cti-research.md`** — `model: claude-sonnet-5` (was the `sonnet` alias). Body unchanged.
- **`prompts/cti-run.md` Phase 5.7** — one definition every iteration; decision rules 1–2 keep the confirmation pass and drop the "other model" clause; the spawn-message contract states the iteration's role (`first pass` / `post-fix pass` / `confirmation pass`) and attaches the deltas block after a NEEDS_FIXES only; the blocked-spawn ladder is retry → re-frame → failed spawn (waiver only for a lost confirmation pass), since there is no other definition to switch to. A `(low confidence)` finding still gets a checked decision and a recorded rebuttal when declined. New § Output discipline after § Style rules: instructions apply at the scope they state; written output carries what the reader needs and stops (entry shape, changelog-section shape, run-record notes shape, one paragraph for a quiet window); narration is one line per phase boundary; the gate and the loop are the verification, no re-check passes beyond them. The sub-agent `**Model:**` paragraph, the quality-gate checklist item and hard invariant #11 no longer mention the rotation.
- **`prompts/quality-audit.md`** — Phase 1 truth passes spawn `cti-verification` for every batch; the lockstep-regeneration clause in Phase 4 is gone; Phases 5.5→7 describe the gate without the rotation. New paragraph after the cleanup mission: the audit runs on Opus 5 with Sonnet 5 sub-agents; delegate only the research-class work the prompt names, add no verification passes beyond Phase 5.7, lead every report section with the finding and stop, one-line empty sections, one narration line per phase boundary.
- **`prompts/verification.md`** — Phase 5.7 policy paragraphs and the checklist item describe the single-definition gate.
- **`tools/check_run.py`** — `SINGLE_VERIFIER_FROM = (4, 1)`: the `verification-rotation` check and the same-model WARN in `check_verification_confirmation` apply to v3.23–v4.0 records only; v4.1+ records report `single verifier definition — no alternation required`. The consecutive-CLEAN requirement, the cap-waiver logic and `confirmation_waived` handling are unchanged; v4.0's lifecycle checks are untouched. Messages no longer say "other-model".
- **`site/build.py` / `site/test_build.py`** — `_verification_confirmation` classifies a same-definition confirming pair as `confirmed` on v4.1+ records (`same-model` stays for rotation-era records); the ops chip reads `✓ double-CLEAN · Claude Sonnet 5 ×2` when both passes report the same model; tooltips and the KPI subtitle say "two consecutive CLEAN passes" instead of "two models". Two fixtures added (v4.1 same-definition → confirmed; v4.0 same-definition → same-model).
- **Docs, README and memory** — `docs/pipeline.md` run-record comments, `docs/architecture.md` (agent list, phase diagram — which also still said "iter 5" for a cap that has been 8 since v3.27), `docs/operating.md` (allow-list item 8, troubleshooting rows, env-var examples), `docs/routines.md`, `README.md`, `CLAUDE.md` § Custom sub-agents and § Operational guardrails. `CLAUDE.md` also loses its derivable directory-layout block, two memory-mechanics bullets duplicated in `MEMORY.md`, and the per-agent bullets duplicated in the agent frontmatter; the versioning rule and the intel-run ↔ audit contract move to `.claude/rules/prompt-editing.md`, a path-scoped rules file that loads when a session touches `prompts/**` or `.claude/agents/**`. Memory: new `routine-model-assignment.md`; `model-identity-and-rotation.md` and `classifier-trips-on-spawns.md` carry dated addenda; the index updated.

### What stays

Everything in v4.0 stands — the entry lifecycle, the changelog contract, the retired weekly, the lifecycle gate checks, the migration. The double-CLEAN gate keeps its shape — a CLEAN publish is still the final two iterations both CLEAN, cap 8, low-residual early exit, cap fail-open, `verification.confirmation_waived` for a lost confirmation pass — and `check_run.py` still FAILs an unconfirmed final CLEAN pre-commit. F1–F18 and every parsing contract between verifier, main agent, gate and site are unchanged. Every hard invariant holds; invariant #11's wording drops "model rotation" and nothing else. The v3.15 self-identification protocol (harness prompt line first, env vars as marked fallback) is unchanged; its rotation-failure caveat is simply moot. Rotation-era records (v3.23–v4.0) are judged under their own era's rule, so `check_run.py --all` output and the existing `state/warning_acknowledgments.json` rows are untouched. `cti-run.md` and `quality-audit.md` move to v4.1 in lockstep.

---

## 4.0 — 2026-08-27 (one living entry per finding; the weekly strategic routine is retired)

### Why

Two operator decisions on 2026-08-27, taken together because they describe one product.

1. **The weekly briefing is gone.** The operator wants two routines and only two: the intel run — fired on whatever cadence the operator sets, one a day today, several a day tomorrow — and the quality audit that checks and improves what the intel run publishes. The weekly strategic run (`prompts/weekly-summary.md`, the `/weekly/YYYY-Www/` pages, the `feed-weekly.xml` digest, the W1–W3 research domains, the `duplicate-week` guard and its backup routine) was a third product with its own lens, its own dedup polarity and its own defect classes (2026-W32 and W33 audits: 17 and 51 mis-citation findings respectively, all of the "re-framed a fact it did not fetch" shape). It also carried the two things the intel run still needs — the standing policy / regulatory watch and the ATT&CK pin-freshness duty — which move to the intel run's S2 and to the audit respectively.

2. **Per finding, one entry — updated in place, with a changelog.** v3 made entries immutable and modelled every development and correction as a *second* entry linked by `update_of`. That produced 180 update entries against 114 originals in three months, with three consequences the operator named: a reader following a story hops between files (the original never says the patch shipped; the correction of a weekly's claim lives in another day's folder); an automated triage consumer reading the original sees a stale state unless it also walks the chain; and the live brief never re-surfaces a finding when something material changes about it — the update appears as an unrelated new row. The operator's directive: **one entry per finding, updated with further developments and corrections in that same entry, each update and correction clearly timestamped, a changelog on the entry, a first-published timestamp and an updated timestamp, and the update timestamp floats the entry back to the top of the live page so a reader notices.** Later runs and the audit correct or improve previous findings the same way.

### What changed

**Data model (`docs/pipeline.md` § Entry lifecycle — normative; `site/content_model.py`).**

- Every entry carries `updated_at` (null until the first change; then exactly the last record's `at`) and an append-only `updates[]` changelog: `{at, run_id, type: update | correction | improvement, summary, fields?, merged_from?}`. Each record pairs 1:1, in order, with a body section headed exactly `## <Type> — <at>` carrying the inline-cited delta. `discovered_at` (first publication), `run_id` (originating fire) and the entry id never change. The type table: `update` = a material development (frontmatter moves to the current state — `cves[].status`/`fixed`, `affected_products`, entities, techniques, tags; `actions[]` is *replaced*, never accumulated; priority/`immediate_action` when the bar moves; headline/summary only when a summary-only reader must now know something different); `correction` = the entry stated something wrong (the wrong statement is fixed where it stands AND the section says what was wrong, what is right, and the ground truth); `improvement` = precision or depth added without reversing a claim.
- **No silent edits.** A published entry changes only through a record; `discovered_at` / `run_id` / path never; one record per fire per entry; a wrong earlier record is fixed by a further `correction`, never edited.
- **`update_of` is retired** and FAILs the gate. `horizon: strategic`, `weekly_section`, and the `synthesis` / `outlook` kinds are LEGACY — valid on the archived pre-v4 entries, FAIL on a v4+ entry (`content_model.ACTIVE_KINDS`). `policy` stays a first-class kind (a regulatory obligation change is a PD-11(c) finding) and renders in the brief's Research, Reports & Policy section. Run kinds are `intel` / `audit` (`weekly` legacy).
- The entry's **activity moment** is `max(discovered_at, updated_at)` (`content_model.entry_activity_ts`) — the live brief's sort key. `references[]` doubles as the explicit dedup statement: a genuinely distinct finding sharing a CVE with an older entry lists it, or the gate FAILs the new entry as a duplicate.
- Run records gain `updated_entry_ids[]` (the entries a fire appended a record to; `len == entries_updated`); `entries_published` counts new files only.

**Migration (`tools/migrate_updates.py`, one-shot, kept for provenance).** All 180 `update_of` entries were folded into their 114 root entries as changelog records (`type` from the title — 8 corrections, 172 updates; `merged_from` names the folded id), their bodies became the `## <Type> — <at>` sections, the roots' frontmatter was brought to the state the chain described (per-CVE records replaced by the newer record, sources/evidence/entities/techniques/products/tags unioned, priority escalated where the update escalated it), 48 entries' `references[]` and 16 registry `relations[].source` values were re-pointed to the surviving roots, and the 180 files were removed. The old permalinks stay reachable: `site/build.py` emits a redirect stub at every `merged_from` id.

**Mechanical gate (`tools/check_run.py`).** New `entry-updates` (record shape, `updated_at` mirror, strictly increasing `at`, 1:1 section pairing, record `run_id` resolution, one record per fire), `silent-edit` (an entry modified in the working tree without a record for this run FAILs — and runs never delete entries), `legacy-shape` (no strategic/synthesis/outlook/weekly_section on a v4 entry; run kind intel/audit). Dedup FAILs a new entry whose CVEs overlap ANY existing entry unless declared in `references[]` (was: 14 days only — month-old coverage passed silently); entity overlap inside 14 days still WARNs. Run counters check `entries_updated` + `updated_entry_ids` against disk. The per-run scope is new entries **plus** updated entries (whole entry re-validated; URL liveness on an updated entry only for the sources the run added). The `update-target` chain check is retired with `update_of`. Fix recipes in `prompts/check-run-fixes.md`.

**Site (`site/build.py`, `brief.js`, `styles.css`, `taxonomy.yaml`, tests).** `/live/` and `data/briefbook.json` window and order entries by activity moment: an updated entry appears once, under the run that made the latest record, flagged `UPD` with the record's type and summary; the pulse panel counts updates. Day pages render § Updates to Prior Coverage from the records dated that day. Entry pages show "First published … · Updated …", each changelog section as a timestamped, type-badged block, and a Revision history panel; JSON-LD `dateModified` follows `updated_at`. `feed-items.xml` and the sector feeds emit one item per changelog record (`guid` = `<url>#update-<at>`) besides one per entry; `data/alerts.json` windows by activity and carries `updated_at` + compact `updates[]`. `/weekly/` pages, the weekly index, `feed-weekly.xml`, the weekly nav segment, home card and search kind are removed; the archived strategic entries remain reachable by permalink, entity, CVE, tag/region and search. Redirect stubs for `merged_from` ids.

**Prompts and agents.** `prompts/weekly-summary.md` deleted. `cti-run.md` v4.0: PD-8's update-note rule is now the changelog rule; PD-13's KEV listing ships as an `update` record; the "division of labour with the weekly" is replaced by "the intel run carries the whole operational picture — `policy` is a normal kind, no synthesis product"; Phase 1 gains the `org-policy-watch` managed block under S2 (composed from `config/org-profile.yaml` `policy_watch`); Phase 2 step 5 marks matches as `update` on the existing entry; Phase 4 gains **§ Updating an existing entry (the changelog)** — the six-step procedure; Phase 5.5's validated list, Phase 5.7's scope (updated entries read whole; a bad update is reverted with `git checkout --`, never deleted) and remediation table, Phase 6's staging (`git add -u entries/` — updated entries live under their own dates) and commit template, the quality gates, the output block and hard invariant #19 all follow. `quality-audit.md` v4.0: cadence-agnostic runtime, report filename `<date>-quality-audit.md`, the ATT&CK pin duty, the fix classes rewritten around `correction` / `improvement` records (the immutability-exception ledger is retired — the changelog is the ledger), A-INV-1 and A-INV-5 rewritten. `entry-template.md`: skeleton with `updated_at` / `updates`, a worked "Updating an existing entry" example, no weekly variant. `verification.md`: wording. `cti-research.md`: W1–W3 removed, G1–G3 audit re-sweeps named, S2 owns the policy watch, `novelty: update-of:<entry-id>` means "a changelog record on that entry". `cti-verification.md` (+ alt regenerated byte-identically): scope includes updated entries; new truth check 4c — the changelog contract (same finding, genuine delta, changed fields against sources, record summary ⇔ section, main analysis not contradicting the new state, a silent edit is F4-class); the weekly lens and drop reason removed. `CLAUDE.md`: two routines, the lifecycle hard rule replaces "NEVER edit a published entry", dedup rule, versioning rule for two master prompts, "Intel-run ↔ audit" section, invariants. `tools/compose_prompts.py` targets `cti-run.md` for `org-policy-watch` instead of the weekly. `docs/`, `README.md`, `.claude/memory/` and `state/coverage_backlog.md` follow (the weekly-synthesis and immutability-exception memories become historical).

### What stays

Everything about *what* gets published: PD-1…PD-13, the relevance gate and its sound-AND-complete doctrine, the two-source rule and its carve-outs, no IOCs, English, the Admiralty classification, the ATT&CK mapping rules, the action-item bar, the deep-dive criteria, the anti-crash guards, the Phase 5.7 double-CLEAN rotation, the publishing chain and Phase 7 verification, the zero-warning discipline. Run records stay immutable after their fire with the same two exceptions — they are telemetry about one fire and have no current state to maintain. The audit keeps its truth passes, re-sweeps, systemic review, monthly calibration and warning sweep. Historical strategic entries, weekly run records and audit reports are untouched history. The migrated roots' `discovered_at` and folder dates are unchanged; only their frontmatter state, changelog and body sections grew.

---

## 3.34 — 2026-08-24 (preflight stops trusting the container clock and its first fetch; a "no patch exists" claim must come from the vendor's own channel)

### Why

Two changes from the 2026-08-24T0902Z weekly quality audit (`docs/audits/2026-08-24-weekly-quality-audit.md`), which ran concurrently with — and was overtaken by — the late-promoting 2026-08-23T1311Z audit. A disclosure first: **both audits independently found and fixed the same run-clock defect** (v3.33's "the run clock stops at the end of the run" and the 0902Z audit's identical finding). The 08-23 audit's fix reached `main` first and is the one that stands; the 0902Z audit's duplicate gate check was discarded in the merge rather than shipped twice. What follows is only what the 0902Z audit found that v3.33 does not cover.

1. **A fire can boot with a wrong clock and a stale clone, and nothing notices — it happened twice in one day.** The 2026-08-24T0902Z audit booted with `date -u` reading **2026-08-16T13:13Z** and a first `git fetch origin main` that returned refs eight days old. The two errors corroborated each other, so nothing looked wrong: the fire computed a 168 h window ending 2026-08-16, briefed eight sub-agents on it, and for several minutes the evidence read as an eight-day pipeline outage. Minutes after its mid-run recovery, the 2026-08-24T0906Z intel fire hit the same stale clock and stood down with zero entries. A wrong clock does not fail loudly: it produces a well-formed run that researches, dedups and audits the wrong week — with dedup silently passing everything, because candidates from the true present look new against a stale index.
2. **"No patch exists" was sourced to a news headline.** Three 2026-W33 weekly entries told readers GeoServer's actively exploited `jsonArrayContains` SQL injection had no vendor fix and that taking query endpoints off the public internet "is the whole remediation" — OSGeo had shipped 3.0.1 / 2.28.5 / 2.27.6 on **2026-08-14**, two days before those entries published. Both the 08-23 and 08-24 audits found the defect independently; the 08-23 audit queued the correction, and the 08-24 audit published it (`2026-08-24/correction-geoserver-w33-no-vendor-fix-claim-patch-existed`). A negative claim about patch availability has an expiry date; a relay's snapshot keeps asserting it after it stops being true, and the reader acts on an inverted remediation.

### What changed

- **`prompts/cti-run.md` Phase 0 step 0** — preflight establishes ground truth for "now" and "latest" instead of assuming them. The container clock is cross-checked against an external HTTP `Date` header (multi-host fallback; guarded against the empty-header case, where `date -d ""` silently returns today at midnight and would "correct" a good clock into a bad one; epoch sanity-checked); a skew over 5 minutes hands the run to the network date. The clone is checked by listing the newest run record on `origin/main` and re-fetching before concluding that a suspicious gap is a scheduler outage rather than stale refs. Every spawn message states today's date explicitly, because a sub-agent inherits the same bad clock and cannot detect it alone. The snippet was tested on both paths before shipping; testing it caught two real bugs in its own first draft (a stray CRLF making `date` parse to midnight, and the empty-header case above).
- **`prompts/cti-run.md` Phase 2 item 4b (new)** — a "no patch exists" claim, and every remediation sentence resting on one, must be checked against the vendor's own release / advisory / changelog channel in the run that publishes it, and cited to that check. The vendor's page saying nothing is itself the citation ("no fix listed on the vendor's advisory as of `<date>`") — a different and honest claim. A relay's "unpatched" corroborates the exploitation, never the absence of a fix; the mirror case (a *patched* claim) needs the vendor's version table. Deliberately not mechanised: `no-patch` alongside `patch-available` or a prose `fixed` string legitimately encodes a partially-fixed estate (27 correct records), so a gate check would drown the one defect in false positives.

### What stays

Everything in v3.33 stands — the run-clock fix, `check_run.py`'s `run-clock` gate, the trafilatura capture layer and the notification policy are all the 08-23 audit's work and are untouched. PD-7's gap-derived window and its self-healing are unchanged; the preflight addition makes the *inputs* to that computation trustworthy rather than changing the computation. Entry immutability, the verification loop, and every hard invariant hold. `weekly-summary.md` and `quality-audit.md` move to v3.34 in lockstep.
...
Four changes, each a root cause the 2026-08-09 weekly quality audit confirmed against the window's own records (`docs/audits/2026-08-09-weekly-quality-audit.md`):

1. **A stand-down's verified research had nowhere to go, and nine in-scope items were silently lost.** The 2026-08-03 weekly stood down as `duplicate-week` after researching and verifying the whole ISO week, and listed in its record body nine items the primary weekly did not carry — a CISA/ACSC/NCSC-UK/CCCS joint OT-isolation advisory, the EU AI Act general date of application and the Digital Omnibus amending Article 113, an NCSC UK publication making edge-appliance forensic capability a procurement criterion, Group-IB's `pam_rootok` anti-forensics tradecraft, an updated SBOM minimum-elements baseline, and more. **Not one of them was ever published.** Nothing consumed the list, and both recency gates made them unreachable: the next intel run's window is 24–26 h, the next weekly's is the following ISO week. Every subsequent fire was structurally incapable of picking them up.
2. **The duplicate-week guard cannot see a completed-but-unpromoted primary.** Second consecutive weekly cycle disrupted by the same race. On 2026-08-03 the backup fire passed the 01:10Z Phase 0 guard against `origin/main`, but the primary `2026-08-02T2311Z-weekly` had already *completed* at 00:06Z and was sitting on its feature branch waiting for auto-merge. The v3.30 pre-verifier re-check caught it at 02:10Z — an hour of pipeline the branch sweep would have saved.
3. **A blocked alternate verifier was being treated as "rotation impossible" rather than "try the other route".** The Sonnet-pinned `cti-verification-alt` is intermittently classifier-blocked, and when it is, the whole fire loses its two-model agreement: 2026-07-23 ran all 8 iterations on Opus, 2026-08-06 ran all 5 on Opus after four blocked alternate spawns. Yet the pipeline already knows the recovery — the 2026-08-03 weekly recovered both of its blocked Sonnet-pinned *research* spawns with an explicit `model: opus` override, and the two verifier definitions are byte-identical below their header note, so an overridden spawn is the same verifier on the intended model.
4. **The gate saw none of it.** `check_run.py`'s rotation check lives inside the double-CLEAN branch and returns early on a `NEEDS_FIXES` final verdict, so 2026-08-06's five consecutive same-definition iterations were invisible store-wide — and that run had recorded its waiver at top level as `verification_confirmation_waived` rather than under `verification.`, where the check reads it, so a fully-documented fail-open would have been reported as an undocumented one had the run converged to CLEAN. Separately, two of the window's seven F17 findings were a purely mechanical self-contradiction — `credibility: 1` ("corroborated by other independent sources") on an entry whose own `verification` says `single-source` — costing verifier iterations to catch what the gate can settle for free.

### What changed

- **`state/coverage_backlog.md` (new) + `prompts/cti-run.md` Phase 0 step 5b + `prompts/weekly-summary.md` Phases 5 → 7** — verified-but-unpublished items get a queue. Any fire that verifies a relevant item it cannot publish (a `duplicate-week` stand-down, a watchdog cut, an abandoned sub-agent whose findings survived) appends it to the backlog as well as narrating it; every intel run reads the backlog in Phase 0 and works it down. Backlog items are **exempt from the recency gate** — each carries its own `event_date` and was verified in-window by the fire that surfaced it, so its age reflects a pipeline race rather than staleness — and are subject to every other gate unchanged. Rows are struck on publication (with the entry id) or on a reasoned decision not to publish; never silently dropped. Seeded with the eight recoverable items from the 2026-08-03 stand-down.
- **`prompts/weekly-summary.md` Phases 5 → 7** — both duplicate-week guard points also sweep the unpromoted `claude/**` feature branches (`git ls-remote --heads origin 'claude/*'`), and a completed primary record found there counts as a match, because auto-merge will promote it.
- **`prompts/cti-run.md` Phase 5.7 § Spawn** — a blocked alternate spawn walks a four-rung ladder before the rotation is surrendered: spawn the alternate → retry once → **spawn the other definition with an explicit `model:` override to the missing model** → only then a same-model pass as a recorded exception, with the override attempt itself recorded so the next audit can see the ladder was walked.
- **`tools/check_run.py`** — three additions. The rotation is checked across the **whole** iteration chain on every publish path rather than only at the confirming pair, FAILing consecutive same-definition iterations that no recorded waiver explains (new `verification-rotation` label). The confirmation waiver is honoured at either key placement, with a nudge on fresh runs toward the canonical `verification.confirmation_waived`. And `credibility: 1` on an entry whose `verification` is `single-source*` is now a mechanical FAIL in `check_classification` — a frontmatter self-contradiction the gate can settle without a verifier iteration.
- **`tools/check_run.py` composition report** — the informational rolling-24 h line gains the window's `high` share, actions per operational entry, and the share of entries with no action, each against a trailing-28-day baseline. This is the cheap, observational half of the 2026-08-02 audit's recommendation 4: it gives the operator the two rates F16 and F18 are asked to judge, and **gates nothing**. What that audit left as an operator decision — passing these numbers into the verifier's spawn message, which would shape editorial judgement — is still an operator decision and is not shipped.

### What stays

Entry immutability is untouched: the backlog publishes *new* entries through the full normal gates, and nothing in it licenses editing a published one. The recency discipline is unchanged for everything except backlog rows, whose exemption is narrow and justified by the fact that a prior fire already verified them in-window — PD-7's gap-derived window, its 24 h floor and its self-healing are all as before. The duplicate-week invariant is unchanged; the branch sweep widens what the existing guard can see, it does not change what a match means. Hard invariant #11 is strengthened rather than weakened — the model-override ladder exists to *reach* the two-model agreement the double-CLEAN gate requires, and the same-model fallback remains a recorded exception of last resort, now with the gate FAILing an unrecorded one on every publish path instead of only the CLEAN one. Hard invariant #20 holds absolutely: the new composition rates are informational, no number fails, warns or caps anything, and entry volume remains governed by the relevance gate alone. `weekly-summary.md` and `quality-audit.md` move to v3.31 in lockstep.

## 3.30 — 2026-08-02 (disclosure streams are not campaigns; quote fidelity becomes a literal substring check; ATT&CK completeness gets an evidence floor; republication is not corroboration; CVE provenance covers which flaw an id names; the duplicate-week guard runs again before the verifier loop)

### Why

Six changes, each a root cause the 2026-08-02 weekly quality audit confirmed against ground truth (`docs/audits/2026-08-02-weekly-quality-audit.md`), each with a named failure in the window:

1. **A third consecutive audit recovered a miss from the same Joomla-extension disclosure stream.** 2026-07-18 recovered Moodle local_o365, 2026-07-26 recovered Balbooa Gridbox, and this window's SP Page Builder disclosure (four CVEs, an effectively pre-auth SQL injection reading the whole database) was never covered at all — while the EasyStore and Events Booking disclosures from the same publisher appeared only *inside* a weekly round-up, named without a citation and with no entry, no `cves[]` record and no `/cve/` page of their own. The mechanism was PD-8's long-running-campaign rule, written for campaign activity recurring, being applied to a stream of independent per-product vulnerability disclosures. A defender running product X gets nothing from an entry about product Y.
2. **Quote fidelity is now the dominant residual truth defect, and prose has not fixed it.** Three instances survived the verifier loops in one week: a word spliced in from an adjacent sentence (`octlurk-silklurk-…`), a de-hedged rewrite presented as verbatim (`hugging-face-openai-…`), and a sentence Unit 42 never wrote (`unit42-autonomous-…`) that also dropped 11 confirmed compromises from the campaign's impact count. All three are catchable by a literal substring search against the page the composer had already fetched. The v3.26 natural experiment is the precedent: its mechanically-checkable limb went to zero defects, its judgement-call limb did not move.
3. **The `techniques[]` completeness duty had no evidence floor.** Two entries mapped a technique no cited source supports — `T1567.002` on an incident whose only described attacker action is a leak-site posting, and `T1190` on an incident whose reporting says only that portals "were impacted" — both reaching the `/attack/` matrix and the Navigator exports. The mandatory-non-empty rule for behaviour kinds was being satisfied by inventing the access vector.
4. **Credibility `1` was being awarded for republication rather than corroboration.** Two entries rated a vendor advisory plus national-CERT restatements as independently corroborated. The store already contains the correct reasoning, three days earlier, in its own Airflow entry: two publishers, one assessor, credibility 2.
5. **CVE provenance was read as covering the id and the score but not the pairing.** This audit's own recovered SP Page Builder entry bound three of four identifiers to the wrong flaws: the discloser's TL;DR lists four vulnerabilities in one order and then lists four assigned CVEs in ascending numeric order, and the entry paired them positionally instead of reading the explicit mapping table further down the same page. Wrong CWE and wrong auth level followed, and the inverted mapping reached `state/cves_seen.json` before iteration 1 of the verifier loop caught it. The same page also printed both the discloser's self-scores and the CNA's, stating which governs; the entry took the wrong set.
6. **The duplicate-week guard fires only at preflight.** On 2026-07-27 the backup weekly passed the guard at 01:09Z, composed nine strategic entries, ran eight verification iterations, and discovered the primary's already-published W30 record at the Phase 6 pre-push sync — 2.3 h of wall clock for a stand-down that was correct and could have been reached 90 minutes earlier.

### What changed

- **`prompts/cti-run.md` PD-8(b)** — the consolidation rule is scoped: it governs campaign/actor *activity*, never a stream of independent vulnerability disclosures. Distinct product, distinct CVE set, distinct affected estate ⇒ its own entry under § Item granularity, however many land in a week, with the three-audit evidence named and a mechanical test: *if a round-up entry names a product's vulnerabilities, that product's disclosure needed its own entry.*
- **`prompts/cti-run.md` Phase 4 item 4 (evidence escalation)** — quote fidelity is mechanised: `grep -F` every candidate quote against the body saved to `work/<run-id>/` before the entry is written; no hit ⇒ shorten to the fragment that hits, split into two `evidence[]` records, or drop the quotation marks. Applies to any body text inside quotation marks, not only `evidence[]`. With the extraction caveat that makes the check trustworthy: strip tags to the empty string rather than to a space, or `database</strong>, password` becomes `database , password` and the quote passes locally while failing on the live page. Two shapes are not quotes however faithfully extracted — a table row (cells are not contiguous prose) and your own paraphrase in scare quotes.
- **`prompts/cti-run.md` § Triage-ready behavioral description, ATT&CK clause** — the completeness duty gains an evidence floor: when the cited sources do not state how access was obtained, the entry does not map an access vector; if the honest mapping for a behaviour-kind entry would be empty, fix the entry, never the mapping.
- **`prompts/cti-run.md` § Intel classification** — independence is defined: a second party that observed or assessed, not one that republished. A vendor advisory plus national-CERT restatements is one assessor with several publishers — credibility 2. Same for a wire pickup of a lab report and for an aggregator confirming only that a CVE id exists.
- **`prompts/cti-run.md` Phase 2 step 4** — the per-CVE-authority rule is extended to the pairing: it covers *which flaw an id names*, a positional mapping between two lists on one page is a guess rather than a transcription, and where no explicit mapping exists the ids ship without per-flaw attribution instead of an invented one. Where a discloser publishes its own score alongside the CNA's, the CNA's is the one that travels with the CVE.
- **`prompts/weekly-summary.md` Phases 5 → 7** — the duplicate-week guard is re-run immediately after the pre-verify gate and before the first verifier spawn, with the `git fetch` + `git grep` recipe inline and the stand-down procedure spelled out (withdraw the composed entries, `disposition: duplicate-week`, `entries_published: 0`, one verifier iteration on the record alone, publishing chain).

### What stays

PD-8's dedup discipline is untouched — a genuine repeat is still an `update_of` delta or nothing, and the ≤1-consolidated-update-per-week ceiling still applies to campaign activity; the change only stops that ceiling being applied to a class it was never written for. The evidence rule's substance is unchanged (verbatim still means a contiguous substring, copied not composed) — v3.30 adds the check that proves it rather than a new standard. `techniques[]` remains the canonical, complete mapping surface and remains mandatory-non-empty on `threat`/`incident`/`vulnerability`; the floor narrows what may satisfy it, never what must be captured when the evidence is there. The Admiralty scheme, its code vocabularies and the triage-kind split are unchanged — only the definition of independence is made explicit. The weekly's duplicate-week invariant, its Phase 0 guard, the ≥1-verification-iteration rule and the mandatory run record are all unchanged; the new check is an additional firing of an existing guard, not a new policy. The v3.21 CVE-provenance rule is unchanged in substance — the roundup still never beats the authority, and an unresolvable id still never enters `cves[]`; v3.30 only makes explicit that the pairing is part of what the authority owns. All hard invariants hold. `quality-audit.md` moves to v3.30 in lockstep with no body change.

## 3.29 — 2026-07-26 (KEV listing vs deadline split; candidate sources rejoin the rotation and promotion becomes mechanical; per-fact attribution gets a per-citation adjacency check; PD-11(b)'s "otherwise" limb defended; CSAF fixed-version transcription)

### Why

All five changes are root causes the 2026-07-26 weekly quality audit confirmed against ground truth (`docs/audits/2026-07-26-weekly-quality-audit.md`), each with a named failure in the window:

1. **A confirmed coverage miss on actively-exploited WordPress core.** CISA KEV-listed the WP2Shell pair (CVE-2026-63030 / CVE-2026-60137) on 2026-07-21, moving them from "no confirmed in-the-wild exploitation" — the words in the store's own 07-18 entry — to confirmed exploited. The 07-22 fire saw the KEV batch, published the Langflow CVE from it, and dropped the WordPress pair citing PD-13's "a KEV listing never opens an update entry" plus a premise ("already reported as actively exploited") that contradicted its own prior entry. Its verifier flagged the gap as a missed angle; the drop reasoning overrode it. PD-13 attaches "never opens an update entry" to the *remediation deadline*, but the clause was read onto the *listing flag* — which the same directive defines as exploitation confirmation.
2. **A dead source-lifecycle rule.** 11 of 27 candidate sources had met the "promote after 3 contributing runs" bar, one cited by 11 distinct runs; none had ever been promoted, because no fire can count earlier fires and nothing computed it. Separately, `wordpress-org-news` — added by the previous audit specifically to close the discovery gap that caused the WP2Shell miss — was never fetched once in eight runs, its `last_successful_fetch` still `null`, because candidates are absent from the digest's `active_ids` and the allocation rule never said they rotate.
3. **Per-fact attribution is the pipeline's dominant residual defect class.** 11 of the audit's 25 truth findings were "true fact, wrong cited source" (9 imprecisions plus 2 of the 3 factual errors), spread across operational entries and the weekly, surviving verifier loops of 3–8 iterations. The v3.26 weekly fix proved the pattern: its mechanically-checkable half (citation dates) went from 12-of-15 entries drifting to **0 of 52 citations wrong**; its judgement-call half (does this source carry this fact) stayed flat at 4 defects per batch.
4. **PD-11(b)'s "otherwise requiring an out-of-band response" limb had collapsed to "exploited or nothing"** — two consecutive audits recovered unexploited flaws whose mechanics forced the timeline (2026-07-18: Moodle local_o365 JWT forgery; 2026-07-26: Balbooa Gridbox cookie-forgery to Joomla Super User). The watch item opened on 07-18 set exactly this condition for shipping a clarification.
5. **A memory-only rule that did not reach composition.** An OT/ICS entry shipped five CVEs with `fixed: "not stated in advisory"` when the advisory's own CSAF remediation block and the linked GHSA both named the fixed release — the guidance existed in `.claude/memory/csaf-msrc-transcription.md` but not in the prompt.

### What changed

- **`prompts/cti-run.md` PD-13 restructured** into the deadline half (unchanged: never justifies priority, never opens an update entry, never frames an action) and the listing half (a KEV addition that moves an already-covered CVE from not-confirmed-exploited to confirmed-exploited **is** an exploitation-status change and ships as an `update_of` delta; adding `cisa-kev` to a CVE already described as exploited ships nothing). New duty: before dropping a KEV addition as already-covered, **re-read the covered entry's own `cves[].status` and summary** rather than trusting recall of it.
- **`prompts/cti-run.md` Phase 0 allocation** — new text in rule 2 (`status: candidate` records rotate with their domain's standard tier; absent/`null` `last_successful_fetch` sorts oldest, so a source added last run tops the next slice; `active_ids` is not the allocation list) and new rule 4 (act on `sources.promotion_due` from the digest, flip to `active`, record in `sources_changed[]`). Phase 5 lifecycle line now points at the digest for the count.
- **`tools/run_summary.py`** — new `sources.promotion_due[]` in the digest (`{id, contributing_runs, last_run_id}`): counts distinct `run_id`s among published entries whose frontmatter `sources[]` cites each candidate's host or a subdomain of it, threshold via `--promote-after` (default 3). This is what makes the promotion rule executable by a stateless fire.
- **`prompts/cti-run.md` Phase 4 item 5** — per-fact attribution gains two mechanical habits: **one citation per clause, not per sentence** (a trailing citation silently claims the whole sentence), and **never chain two distinct vulnerabilities, CVEs or incidents inside one identifier-labelled clause**.
- **Both verifier definitions (lockstep)** — truth check 2 gains (d) a per-citation **adjacency check**, explicitly not a sample: for every inline citation, confirm that page carries each fact in the clause it terminates, with the two recurring shapes named (detail belonging to the other co-cited source; count or as-of date spliced onto another figure's date) and identifier-mismatch binding called out; and (e) citation date vs the source's own publication metadata, with a 1-day UTC-rendering tolerance.
- **`prompts/weekly-summary.md`** — the v3.26 duty carries a recorded effectiveness verdict (limb (a) took, limb (b) did not) and limb (b) is now mechanical: one citation per clause, no two-vulnerability chaining under one identifier, and quotation marks mean a contiguous verbatim substring.
- **`prompts/cti-run.md` PD-11(b)** — the "otherwise" limb is defended by name: a flaw with no exploitation signal still clears (b) when its own mechanics force the timeline (anonymous single-request path to full administrative control of internet-facing infrastructure; a trivially-rediscoverable bug whose fix diff hands over the technique; a disclosure whose sibling flaws in the same wave were weaponised within days), stated in one clause. "Absence of exploitation is not evidence of safety when the exploit is a cookie value."
- **`prompts/cti-run.md` Phase 2 step 4** — read `affected` / `fixed` from structured fields (CSAF `product_status`, `remediations[].vendor_fix`, or the vendor's version table), never the prose summary.

### What stays

PD-13's substance is intact — foreign-jurisdiction compliance deadlines remain non-signal, and this changes nothing about priority (a KEV listing still never justifies `critical`/`high` by itself; the recovered WP2Shell delta ships at `high` on exploitation and exposure, not on the KEV flag). PD-11's conservatism is unchanged: relevance doubt still resolves toward drop, the routine patch cycle is still out of scope, and the clarified limb narrows rather than widens what qualifies by demanding a named mechanical reason. Volume stays governed by the gate with no target or ceiling. The source lifecycle's thresholds, the one-new-candidate-per-run cap, the never-demote-on-403 rule and append-only notes are untouched — only the counting is now mechanical. Verifier scope, F-code vocabulary, return contract, 30-min cap, rotation and the double-CLEAN publish gate (cap 8) are unchanged; the adjacency check tightens F3's existing definition rather than adding a category. All hard invariants hold. `quality-audit.md` moves to v3.29 in lockstep with no body change.
...
Operator-directed, in same-day response to the 2026-07-18 weekly quality audit (`docs/audits/2026-07-18-weekly-quality-audit.md` — operator response recorded as an addendum there). Three decisions: (1) the audit's recommendation 2 documented that half the post-v3.23 runs were publishing on `confirmation_waived` at the 5-iteration cap — the double-CLEAN confirmation gate stays required, but the loop gets room to converge instead of churning into the fail-open; (2) the audit's recommendation 1 flagged the post-07-14 drop to a single daily fire as a possibly-dead scheduler slot — the operator confirmed it is intended, and made the general policy explicit: cadence is tuned at will, the pipeline works off the gap to the last run, and a cadence change is never a defect; (3) the `bd.zh.ch` MedusaLocker watch item is closed by operator decision — the normal run pipeline, not audit tracking, owns any future development of that story.

### What changed

- **Phase 5.7 verifier iteration cap 5 → 8, everywhere normative:** `cti-run.md` (anti-crash guard #2 rationale, decision rule 6 "Iteration 8 without a publishable outcome", loop-shape bullet "Cap 8 iterations", hard-invariant checklist item 11 "≤8 iterations"); `weekly-summary.md` and `quality-audit.md` shared-machinery references ("cap 8"); `prompts/verification.md` (preflight checklist ≤8, gate paragraph, loop heading); **both verifier definitions in lockstep** (frontmatter description, gatekeeper block "capped at 8 iterations", padding rule "8-iteration ceiling", context-budget note, loop-contract "Hard cap 8 iterations"); `tools/check_run.py` `VERIFIER_ITERATION_CAP` 5 → 8 with a version gate (`CAP_EIGHT_FROM = (3, 27)`) so the "first CLEAN landed at the cap" waiver check keeps 5-cap semantics for pre-v3.27 records; `site/build.py` cap-breach tooltip de-hardcoded ("at the final iteration" instead of "at iteration 5"); CLAUDE.md, README.md, docs/routines.md, docs/architecture.md. **The double-CLEAN gate itself is unchanged** — two consecutive CLEAN verdicts on two different models still gate every CLEAN publish; only the safety-valve ceiling moved.
- **Scheduler cadence policy (`quality-audit.md` Phase 3 telemetry item; `cti-run.md` runtime banner; CLAUDE.md):** the cadence is operator-owned and intentionally variable — more fires, fewer fires, different slots, changed at will. The pipeline is gap-driven (PD-7, unchanged — it already did this); what the audit checks is that the gap-derived windows self-healed across a change with no coverage hole. A cadence change is NOT a finding and NOT an operator recommendation; only a record-less gap on a schedule that was supposed to fire remains an availability signal. Memory: `.claude/memory/scheduler-and-workflow-races.md` + index carry the decision.
- **Operator watch-item closures are final (`quality-audit.md` Phase 0 step 3):** a watch item the operator explicitly closed (audit-report operator-response addendum, or a dated memory note) gets no re-check duty and is never re-opened by an audit; new information on the underlying story flows through the normal intel runs (as `update_of` where an entry exists). Applied: `bd.zh.ch` (Kanton Zürich Baudirektion MedusaLocker listing) closed in the 07-18 report's addendum — open watch-item ledger drops to 3 + 1 fix-effectiveness check.

### What stays

The double-CLEAN publish gate (v3.23) in full: rotation, confirmation pass on the other model, `confirmation_waived` bookkeeping, low-residual early exit, `verification_residual_count` semantics, ≥1 iteration always / ≥2 for a CLEAN publish. The 30-min verifier and 45-min research caps. PD-7's gap-derived window formula — untouched; this release only names the policy it already implements. Entry immutability (the closed watch item's entry and registry record stay as published), F1–F18, the mechanical gate, feature-branch-only publishing, relevance discipline, run-record-per-fire, memory commits. Pre-v3.27 run records stay green (version-gated cap check, same pattern as v3.18/v3.21/v3.23). All three master prompts move to v3.27 in lockstep; both verifier bodies regenerated byte-identically.
...
Operator-directed. The Phase 5.7 publish gate rested on a single verdict: whichever model happened to run the iteration that returned CLEAN decided publication alone, so one model's blind spot (the very thing the opus/sonnet rotation exists to catch on the NEEDS_FIXES path) was never challenged on the CLEAN path — an iteration-1 CLEAN published without any second model ever reading the run. The fix makes the rotation carry the publish decision too: a CLEAN is a hypothesis until the next iteration — by rotation always the other model — independently confirms it. Alongside, the operator asked for the run record's verification & coverage notes (borderline drops, judged-not-relevant items, dedup decisions, coverage gaps) to be reachable directly from the Live view, so a reader can check any day what a run dropped and why, without digging through the repo.

### What changed

- **Phase 5.7 publish gate (`cti-run.md`):** the gate to publish is now a **confirmed CLEAN — two consecutive iterations, on two different models, both returning verdict CLEAN**. Decision rules rewritten: rule 1 publishes only a confirmed CLEAN; new rule 2 spawns the other-model **confirmation pass** after any unconfirmed CLEAN (it reads cold, carries full verdict weight, and a NEEDS_FIXES from it re-enters the fix loop, restarting the CLEAN chain); the low-residual early exit (now rule 5) and the 5-iteration cap (rule 6) stay the fail-open valves. A first CLEAN landing exactly at the cap publishes unconfirmed as a recorded fail-open. Never spawn the same definition twice in a row; a genuinely blocked other-model spawn (retry first — 2026-06-05 classifier-block precedent) may confirm same-model as a recorded exception.
- **New run-record field `verification.confirmation_waived`** (optional string, docs/pipeline.md § Run records): set ONLY when a run publishes on a CLEAN no second model confirmed (single CLEAN at cap, watchdog-overrun single iteration per anti-crash guard #10, blocked spawn) — the reason, verbatim.
- **`tools/check_run.py` — new `verification-confirmation` check (v3.23+ records):** an unconfirmed final CLEAN FAILs the pre-commit gate unless `confirmation_waived` or the cap explains it (→ WARN); a same-model confirmation WARNs; NEEDS_FIXES finals (early-exit/cap residual path) are untouched. `--all` re-checks store-wide at WARN severity (immutable history). Constants `DOUBLE_CLEAN_FROM = (3, 23)`, `VERIFIER_ITERATION_CAP = 5`.
- **Both verifier definitions (byte-identical bodies regenerated):** gatekeeper section now explains the two CLEAN signatures (first CLEAN vs confirming CLEAN) and that a confirmation pass is not a rubber stamp; verdict block and loop paragraph updated; frontmatter descriptions updated in both.
- **Spawn-message contract:** a confirmation pass after a CLEAN passes no deltas — only the fact that the previous iteration was CLEAN with zero findings — so the second model anchors on the run's output, not the first model's judgement.
- **`prompts/verification.md`:** gate sentence, rotation paragraph, checklist line and loop heading updated to the double-CLEAN contract.
- **Weekly + quality-audit prompts:** banner bumps in lockstep; their Phase 5.7 references updated (weekly's machinery recap line; audit's loop-scope line now reads "≥2 for a CLEAN publish").
- **Site — per-run detail pages `/runs/<run-id>/` (`site/build.py` `render_run_detail_page` + `run_url_path`):** every run record gets a permanent page carrying the full Ops telemetry panel (sub-agent allocation, per-iteration verification verdicts + findings, sources changed, coverage gaps, bridge invocations) plus the record's narrative body — the verification & coverage notes where borderline drops and judged-not-relevant items live. The Live view's run dividers now link there (timestamp label becomes an anchor — same markup/classes, no other Live UI change; mirrored in `brief.js`), and the Ops dashboard's 30-panel in-page run selector is replaced by this page family: `/ops/` keeps only the latest run's panel inline, while run selection lives in the Run log history (every row links its run's page; a jump-to `<select>` navigates the 30 most recent — `app.js` `wireOpsRunPicker` is now a navigator, with legacy `#run=<id>` deep links redirecting). Tests in `site/test_build.py` § run detail pages.
- **Docs:** CLAUDE.md (sub-agent blurb + verification guardrail), README (Phase 5.7 + assurance bullet), docs/architecture.md, docs/routines.md, docs/operating.md (new `verification-confirmation` signal row), docs/pipeline.md (verification block example shows the confirmation iteration + `confirmation_waived`).

### What stays

The rotation itself (odd = Opus `cti-verification`, even = Sonnet `cti-verification-alt`), fresh spawn per iteration, the 5-iteration cap, the low-residual early exit (it publishes on a NEEDS_FIXES final with residuals logged — the double-CLEAN gate governs only the CLEAN path), `check_run.py` green between iterations, ≤3 follow-up research sub-agents per iteration, the 30-min verifier cap, F1–F18, the return contract, and `verification_residual_count` semantics — all unchanged. Verification still never blocks the run record. Pre-v3.23 records stay green (version-gated enforcement, same pattern as v3.18/v3.21). The Live view's UI is unchanged beyond the run label becoming a link; day pages and briefbook.json shapes are untouched.
...
Operator-directed. The rendered brief's § Action Items is the union of every in-window entry's `actions[]`, and with each entry shipping 2–5 actions a ~16-entry day produced a 40–60-item task list — far too long for an on-shift team to actually work, and diluted with items that were not tasks at all: standing detection-engineering ideas, general hardening dressed in product names, and restatements of the body's detection guidance. A list nobody can finish is a list nobody starts — the genuinely urgent patch-and-terminate item drowns between technique-class hunting suggestions and process reminders. The operator's direction: action items must be only what the team should immediately do, concrete and derived from the specific finding, and it is explicitly fine for an entry to carry none; the same quality-over-quantity discipline applies to the whole brief — complete over the relevant signal, but never overloading the analyst, because a finding that isn't analysed in detail might as well not have been published.

### What changed

- **New Phase 4 § `actions[]` — the do-now bar** (`prompts/cti-run.md`). An action ships only when ALL hold: (1) **concrete and self-contained** — exact product / version boundary / config surface / log source from the entry's own cited facts, executable without re-reading the entry; hedged phrasing ("consider", "where applicable") disqualifies; (2) **derived from this finding, not from good practice** — the test is *"would this sentence be equally true if this entry had never been published?"*; yes ⇒ generic advice, never an action, not even dressed in product names; (3) **do-now urgency** — started this shift/week (patch/mitigate an exploited exposure, terminate/rotate what the mechanics say is compromised, a bounded compromise-check for the specific artifact); standing detection ideas and open-ended technique-class hunts are body content. **Zero actions is the correct output for a large share of entries** (awareness/research, out-of-nexus lesson entries, most updates); more than three on one entry is near-certain body restatement; no verbatim repeats of an earlier in-window entry's action. The frontmatter bullet now points at the section; the quality-gate checklist gains an `actions[]` line.
- **Inclusion and action items decoupled** (PD-11 calibration): the borderline call's "yes ⇒ include" no longer implies an `actions[]` line — what the responder would do differently lives in the body's `**Defender takeaway:**`, and reaches `actions[]` only when it clears the do-now bar. Added the attention-economics rationale: analyst attention is the resource the brief spends; soundness (no marginal item, no generic action) protects it, completeness (no missing relevant item) earns it — the fix for overload is always to cut the marginal, never the relevant.
- **Verifier: new finding category F18 `action-item-discipline` (editorial)** — new check 10b + report section + verdict-count and slug updates in `.claude/agents/cti-verification.md`; flags generic advice, body restatement, hedged non-tasks, in-window duplicates, and padded lists (> ~3), with an explicit "an empty `actions[]` is never a defect — never ask for an action to be added". `cti-verification-alt.md` regenerated byte-identical below its header note (both headers now say F1–F18); `prompts/verification.md` checklist + editorial-gate + F-range updated; `docs/routines.md` and `CLAUDE.md` references updated.
- **Mechanical gate**: `tools/check_run.py` gains a WARN-severity `action-items` check for v3.19+ runs — flags >3 actions on one entry, canonical generic-advice phrases ("enable MFA", "patch regularly", …), and verbatim duplicate actions across the run's entries. WARN, not FAIL: genericity is an editorial judgment (F18 owns it); the mechanical check only surfaces the textbook cases. Pre-v3.19 records are exempt (entries are immutable).
- **`prompts/entry-template.md`**: skeleton now shows `actions: []` as the default with the do-now bar in the comment, instead of a filled example implying every entry carries actions.
- New CLAUDE.md hard rule (**ALWAYS hold `actions[]` to the do-now bar — empty is the normal case**) so every session — routine or interactive — carries the discipline.
- Banner bumps: `prompts/cti-run.md` + `prompts/weekly-summary.md` → v3.19 in lockstep (the weekly inherits the bar for any operational-kind entries it produces; its strategic lens is unchanged).

### What stays

The inclusion gate itself (PD-11) is untouched in both directions: the brief stays **sound AND complete** — no marginal item survives, no relevant item is dropped, and there is still no entry count, target, or ceiling anywhere; this change governs what an included entry puts into `actions[]`, never whether it is included. Bodies keep their full detection / hunting / hardening depth (Detection clause, `**Defender takeaway:**`, `**Triage:**` — that is where cut action content goes, not into the void). No published entry was edited (immutability intact) — the pre-v3.19 store keeps its historical action lists and stays green under `--all`. The critical/immediate_action bar, deep-dive selection, dedup polarity, triage-ready composition rules, ATT&CK mapping discipline, classification rules, and all hard invariants: unchanged.

## 3.18 — 2026-07-10 (always rated, always mapped: no entry ships without a classification or — on behavior kinds — an ATT&CK mapping; gate FAILs enforce it)

### Why

Operator-directed. The NATO Admiralty classification and the ATT&CK mapping are the two signals a reader (human or triage agent) uses to judge *how much to trust* an item and *what behavior it describes* — yet both were structurally optional. Because this deployment configures no vulnerability-triage scheme, every `vulnerability` entry fell through the triage-kind exemption and shipped with NEITHER rating (`classification: null` + `org_triage: null`) — the exemption was designed for a scheme that doesn't exist here, leaving the most action-driving entry kind permanently unrated. And `techniques[]` was allowed to be "empty when the entry genuinely maps none" with only a WARN behind it, although a `threat`/`incident`/`vulnerability` entry *always* has a mappable access or exploitation vector — an empty mapping on those kinds is a composition defect, not a judgment call, and it leaves the entity TTP profiles, the `/attack/` matrix and the Navigator exports blind to that entry. WARN-severity enforcement means drift accumulates silently; the operator's direction is that every future report and every derived entity profile carries both signals, always, mechanically guaranteed.

### What changed

- **Policy: the triage-kind exemption exists only while a triage scheme does.** When the org profile configures `vulnerability_triage.categories`, triage kinds carry `org_triage` + `classification: null` exactly as before. When none is configured (this deployment), triage kinds carry the Admiralty `classification` block like every other kind — the Admiralty code rates the *reporting* (vendor PSIRT = A, credibility from corroboration), which fits a vulnerability disclosure exactly. **Every entry ships with exactly one rating — never zero.** `tools/compose_prompts.py` renders the deployment-resolved rule into the org-data blocks (both master prompts + research agent) and the verifier context, with selftest coverage for both branches; `prompts/cti-run.md` § Org-triage / § Intel classification and `prompts/entry-template.md` state the fallback.
- **Policy: behavior kinds always map.** `threat` / `incident` / `vulnerability` entries must carry a non-empty, evidence-bound `techniques[]` — at minimum the access or exploitation vector (exposed-service RCE → T1190, phishing lure → T1566, LPE → T1068, …); `research` / `annual-report` should map unless the piece genuinely carries no TTP content; `policy` / `synthesis` / `outlook` may map nothing. Completeness of *evidence-supported* mappings, never invention — every id still names a behavior the body describes and a source supports. Phase 4 § Triage-ready rule 2, the frontmatter contract, the quality-gate checklist, hard invariant 17, `prompts/entry-template.md` and the research agent's findings-YAML contract all state it.
- **Mechanical gate (`tools/check_run.py`), version-gated at v3.18** — for runs whose `prompt_version` ≥ 3.18: a missing rating (`classification` on required kinds, `org_triage` on scheme-owned triage kinds, incl. unknown category ids) and an empty `techniques[]` on a behavior-kind entry are **FAILs**; pre-3.18 records keep WARN severity (entries are immutable — history stays green, the future is gated). Out-of-vocabulary codes remain FAILs at any version. `--all` gains a `store-ratings` sweep that re-applies the enforced checks to every entry of every v3.18+ run forever, so the whole-store validation permanently guarantees "everything is classified". New fix recipes in `prompts/check-run-fixes.md` (`classification` missing-rating, `org-triage`, `attack-mapping` empty-techniques rows).
- **Verifier (F11/F17 scope, both definitions in lockstep)** — F17 restated around "every entry carries exactly one rating — never zero" with the deployment-resolved rule read from § Organization context; check 4b flags an empty `techniques[]` on a behavior-kind entry as F11 (gate-bypass safety net). `cti-verification-alt.md` regenerated byte-identical below its header note.
- **`prompts/verification.md`** — quality-gate checklist lines for classification (never zero ratings) and techniques (never empty on behavior kinds) updated to the same rule.
- Banner bumps: `prompts/cti-run.md` + `prompts/weekly-summary.md` → v3.18 in lockstep (the weekly inherits everything — its `weekly-vuln-rollup` entries are `vulnerability`-kind and now carry the Admiralty block; strategic kinds keep their exemptions).

### What stays

No published entry was edited (immutability intact) — the 894-entry pre-3.18 store keeps its historical rating coverage and stays green under `--all` (WARN at most). The rating vocabularies, the Admiralty axes-independence rule, the evidence-bound mapping discipline ("omitted, never invented" for anything the sources don't support), the F16/F17 truthfulness scope, the configured-scheme behavior (org_triage owns triage kinds the moment a scheme is added to the config), the pinned-dataset lifecycle, and all twenty hard invariants: unchanged. A deployment that configures a triage scheme in `config/org-profile.yaml` gets the original split back automatically via `compose_prompts.py --write` — the fallback is a property of the empty config, not a removal of the scheme.

## 3.17 — 2026-07-09 (versioned MITRE ATT&CK layer: pinned dataset, entity/CVE TTP derivation, Navigator-style overlap matrix, ATT&CK-in-metadata-not-prose)

### Why

Operator-directed. The pipeline talked *about* ATT&CK without being *mapped to* it: `techniques[]` existed in the schema (v3 data model) but zero of 888 published entries carried it — every mapping lived as inline T-ids in prose, invisible to machine consumers, entity pages, and any TTP-level analysis. There was no pinned ATT&CK release to validate ids against (v19 renamed Defense Evasion to Stealth + Defense Impairment and added TA0112 — ids and tactics genuinely drift), no way to see an actor's or vulnerability's technique profile, and no way to compare TTP overlap between actors the way an ATT&CK Navigator layer does. Editorially the old rule pushed ids *into* prose ("woven, never listed"), which cluttered the brief text with numbers most readers skim past while still leaving the machine layer empty — the operator's direction inverts it: keep ATT&CK out of the running text unless essential, make the metadata complete, and derive everything else from there.

### What changed

- **Pinned, versioned ATT&CK dataset** — new `attack/enterprise-attack.json` (contract: `attack/README.md`), a compact extraction (technique id → name, tactics, first-paragraph definition, sub-technique parentage, platforms, lifecycle flags with `revoked_by` forwarding — the ATT&CK analogue of registry tombstones; revoked/deprecated ids are kept so immutable entries keep resolving) from the official `mitre-attack/attack-stix-data` STIX releases, pinned at **v19.1**. New `tools/attack_data.py` owns its lifecycle: `--check` (local vs upstream latest via the official index.json), `--update [--version X.Y]` (rewrite + printed change summary for the commit body), `--selftest` (offline invariants), `--info`.
- **Derived, evidence-bound TTP mappings** — `site/content_model.py` gains `load_attack_dataset` / `resolve_technique_id` / `entry_technique_ids` (frontmatter ∪ in-prose T-ids, dataset-filtered, revoked-forwarded — prose extraction is permanent because the 888-entry legacy store is immutable and carries its mappings in prose only). `site/build.py` aggregates per entity AND per CVE: technique → supporting entry ids, never an assertion without a published entry behind it.
- **Site surfaces** — entity/CVE pages gain an ATT&CK section (techniques grouped by tactic in official matrix order, definitions from the pin, evidence links, lifecycle badges) + a per-entity **ATT&CK Navigator layer export** (`entities/<key>/attack-layer.json`, layer format 4.5, importable into the official Navigator). New **`/attack/`** page: the full enterprise matrix, cells heat-shaded by store-wide coverage, a per-technique definitions-and-evidence directory (no-JS fallback), and a client-side **multi-entity overlap view** (`assets/js/attack.js` + `data/attack.json`) with union / overlap≥2 / common-to-all modes, per-entity color dots, URL-shareable selections (`?sel=…`), and Navigator-layer export of the current comparison. `techniques[]` added to `data/briefbook.json` + `data/alerts.json` records; covered techniques joined the search index; ATT&CK link in the site nav.
- **Mechanical gate** — `tools/check_run.py` FAILs a missing/invariant-violating dataset (`attack-dataset`) and WARNs (`attack-mapping`) on techniques[] ids unknown/revoked/deprecated in the pin and — run scope only — on prose-mapped ids missing from `techniques[]`.
- **Editorial inversion (`prompts/cti-run.md` → v3.17)** — Phase 4 rule 2 rewritten from "ATT&CK woven, never listed" to **"ATT&CK in metadata; prose only where essential"**: `techniques[]` is the canonical, complete mapping surface (active ids per the pin); prose describes behavior in plain language and must read complete without T-numbers; inline ids only where they earn their place (deep-dive kill chains, mapping-as-the-finding, searched-by terms of art); bare ID lists remain a defect, as does a frontmatter id with no described behavior. Frontmatter contract + quality-gate checklist updated to match. `prompts/entry-template.md` worked example + skeleton comments aligned. `.claude/agents/cti-research.md` § Technical depth updated (active ids from the pin; composer keeps ids in metadata). `prompts/weekly-summary.md` → v3.17 in lockstep + new weekly-only maintenance duty: `attack_data.py --check` every weekly fire, result in the run record, update-or-surface on drift.

### What stays

No published entry was edited (immutability intact) — legacy prose mappings are *derived*, not migrated. `TECHNIQUE_ID_RE` format validation unchanged (still a FAIL); the new dataset checks are WARNs except dataset presence/integrity. Verifier definitions (F1–F17) untouched — frontmatter⇔body agreement was already F-scoped, and the mechanical mirror check is `check_run.py`'s job. No-IOC rule, two-source verification, relevance gate, dedup ladder, entity-registry namespace: unchanged. ORG-PROFILE blocks untouched (banner-only lockstep bump).
