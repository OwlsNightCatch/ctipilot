...
## 3.32 — 2026-08-24 (the completion timestamp covers the whole fire; preflight stops trusting the container clock and its first fetch; a "no patch exists" claim must come from the vendor, not a headline)

### Why

Three root causes, each confirmed against the window's own records by the 2026-08-24 weekly quality audit (`docs/audits/2026-08-24-weekly-quality-audit.md`):

1. **Every `duration_seconds` in the store understated its fire, and the wall-clock watchdog was checking that number.** v3.21 added the ~3 h watchdog and taught `check_run.py` to WARN past the runaway threshold — but the value it reads was stamped in **Phase 5**, before the mechanical gate and before the Phase 5.7 verifier loop. **the majority of stored records have a `completed` that precedes one of their own children's `ended_at`** (three independent recomputations gave 103 of 146, 100 of 141 and 104 of 148, moving with the denominator definition), worst skew 125 min (`2026-08-04T0411Z-intel`). `2026-08-10T0411Z-intel` records 3103 s (~52 min) for a fire whose own notes place it at ~2 h 55 m and whose last verifier iteration returned at 06:58Z against a `completed` of 05:02Z. The watchdog therefore had no machine-auditable signal that could see an overrun, and every audit line reading "no runaway this window" was reading a floor rather than a measurement. This defect had already been *diagnosed once and not fixed*: the 2026-08-10 weekly's own verifier caught the shape on that record ("a `main.ended_at` stamp taken at the end of the state phase"), the run corrected its own frontmatter, and the mechanism stayed broken for every fire after it.
2. **A fire can boot with a wrong clock and a stale clone, and nothing notices.** The 2026-08-24 audit fire itself started with `date -u` reading **2026-08-16T13:13Z** and a first `git fetch origin main` that returned refs eight days old. It computed a 168 h window ending 2026-08-16, briefed eight sub-agents on it, and for several minutes the evidence read as an eight-day pipeline outage. Nothing failed loudly — a bad clock produces a well-formed run that researches, dedups and audits the wrong week, and sub-agents inherit the same clock and cannot detect it alone.
3. **"No patch exists" was sourced to a news headline.** Three 2026-W33 weekly entries published 2026-08-16T23:5xZ told readers GeoServer's actively exploited `jsonArrayContains` SQL injection had no vendor fix, and that taking query endpoints off the public internet "is the whole remediation". OSGeo had shipped 3.0.1, 2.28.5 and 2.27.6 on **2026-08-14**, two days earlier, explicitly to resolve it. All three inherited the framing from a 2026-08-14 article titled "…Unpatched GeoServer Zero-Day" that was already stale when written, and none went to `geoserver.org`. A negative claim about patch availability has an expiry date; a relay's snapshot keeps asserting it after it stops being true, and the reader acts on an inverted remediation.

### What changed

- **`prompts/cti-run.md` Phase 5 + Phase 6 step 0** — the Phase 5 `main.ended_at` stamp is explicitly **provisional**, and Phase 6 re-stamps it immediately before staging, after the verifier loop has closed. `completed` and `duration_seconds` now cover the whole fire. Phase 7's polling stays outside the figure by design: `completed` is when the run finished producing, `publish_checked_at` is when it confirmed the result landed.
- **`tools/check_run.py` `check_completion_covers_run` (new)** — FAILs a v3.32+ record whose `completed` precedes any `verification.iterations[].ended_at` or `sub_agents.*.ended_at` it carries, naming the worst offender and the skew; WARNs under `--all`. Scoped to v3.32+ records: the pre-rule records are immutable history, and reporting them would add roughly a hundred warnings whose only resolution is the acknowledgment ledger.
- **`prompts/cti-run.md` Phase 0 step 0** — preflight establishes ground truth for "now" and "latest" instead of assuming them. The container clock is cross-checked against an external HTTP `Date` header (multi-host fallback; guards against the empty-header case, where `date -d ""` silently returns today at midnight and would "correct" a good clock into a bad one); a skew over 5 min hands the run to the network date. The clone is checked by listing the newest run record on `origin/main` and re-fetching before concluding that a suspicious gap is a scheduler outage rather than stale refs. Every spawn message now states today's date explicitly.
- **`prompts/cti-run.md` Phase 2 item 4b (new)** — a "no patch exists" claim, and every remediation sentence resting on one, must be checked against the vendor's own release / advisory / changelog channel in the run that publishes it, and cited to that check. The vendor's page saying nothing is itself the citation ("no fix listed on the vendor's advisory as of `<date>`") — a different and honest claim. A relay's "unpatched" corroborates the exploitation, never the absence of a fix. The mirror case is covered too: a *patched* claim needs the vendor's version table.
- **`sources/sources.json`** — `mysites-guru` moved from `fetch_method: jina` (with `rss_url: null`) to `fetch_method: rss` on the verified `https://mysites.guru/rss.xml`. This is the root cause of a coverage miss **four** consecutive audits have recovered: the original disclosing party for the recurring Joomla third-party-extension RCE stream was pinned to a metered transport whose key pool is fully exhausted, so it was effectively unfetchable.

### What stays

Every hard invariant holds. The runaway threshold itself is unchanged at 3 h — v3.32 fixes the measurement, not the bar, and the honest consequence is that some past fires were slower than their records claim. Entry immutability is untouched: the GeoServer correction ships as a new `update_of` entry and the three W33 entries are not edited. The no-patch rule adds a sourcing duty, not a new gate — it is deliberately **not** mechanised, because `no-patch` alongside `patch-available` is a legitimate encoding for a partially-fixed product estate (Siemens Desigo CC V7/V8/V9, Check Point end-of-support trains) and a check would flag 32 correct records. PD-7's gap-derived window, its 24 h floor and its self-healing are unchanged; the preflight addition makes the inputs to that computation trustworthy rather than changing the computation. `weekly-summary.md` and `quality-audit.md` move to v3.32 in lockstep.
