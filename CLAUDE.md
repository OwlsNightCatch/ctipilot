...
- **NEVER invent a second entity key.** Every actor/campaign/malware/tool/incident/report is linked via its `entities/registry.yaml` key; check aliases before registering anything new; registry keys are permanent (extend aliases, never rename); a discovered duplicate is tombstoned with `merged_into: <canonical-key>` and new entries reference only canonical keys (docs/pipeline.md § Entity registry).
