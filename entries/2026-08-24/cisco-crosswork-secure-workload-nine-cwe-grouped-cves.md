...
sourcing_note: >
  ...
  date records the true first publication. It ships as an update rather than as first coverage because
  this pipeline's W34 weekly rollup, published 2026-08-23 while this fire sat unpublished, reached the
  same advisories from a national-CERT relay and covered them CVE by CVE. Three things in that account
  need correcting from the vendor's own structured data, which is the whole reason this entry still ships:
  the set is nine CVEs, not eight — CVE-2026-20319 is absent from the rollup's enumeration; the
  characterisation of the set as letting unauthenticated attackers execute commands, bypass authentication
  and take full control holds for six of the nine and not for the three that carry PR:L; and the rollup
  records exploitation status as unknown where Cisco states it is not aware of malicious use. The per-CVE
  affected and fixed release strings below appear in neither the rollup nor the relay. The advisories
  themselves were read on 2026-08-22, but `discovered_at` records 2026-08-24 deliberately: what this entry
  now reports is the divergence between the vendor's structured data and the rollup's account of it, and
  that divergence could not be observed until the rollup existed.
confidence: high
update_of: "2026-08-23/weekly-w34-vuln-status-rollup"
