...
updated_at: "2026-08-28T04:55:00Z"
...
updates:
  - at: "2026-08-28T04:55:00Z"
    ...
    type: correction
    summary: >
      This entry's CVE-2026-12537 (Google Gemini CLI) record carried cvss: null and vector:
      user-interaction, sourced only to the OSV mirror of the advisory. The CNA's own CVSS 4.0
      rating, read directly from NVD — labelled "Secondary" in NVD's own type field, despite
      being the assigning CNA's rating rather than NVD's — is 10.0 CRITICAL with User Interaction:
      None (CVSS:4.0/AV:N/AC:L/AT:N/PR:N/UI:N/...); NVD's own "Primary"-labelled CVSS 3.1 rating
      on the same record is 7.8 with User Interaction: Required — a materially different exposure
      picture. The record now carries both scores and the zero-click vector the CNA's own rating
      supports.
    fields:
      - cves
      - body
...
This entry's CVE-2026-12537 record carried `cvss: null` and `vector: user-interaction`, sourced to the OSV mirror of the Gemini CLI advisory rather than to a CVSS score directly. A direct query of NVD's CVE 2.0 API shows the assigning CNA's own rating — which NVD's own record labels type "Secondary" despite it being the CNA's rather than NVD's own score — is CVSS 4.0, 10.0 CRITICAL, with the vector `CVSS:4.0/AV:N/AC:L/AT:N/PR:N/UI:N/...` — User Interaction: None ([NVD/MITRE CVE record, 2026-08-28](https://services.nvd.nist.gov/rest/json/cves/2.0?cveId=CVE-2026-12537)). NVD's own "Primary"-labelled CVSS 3.1 rating on the same record is 7.8, with vector `CVSS:3.1/AV:L/AC:L/PR:N/UI:R/S:U/C:H/I:H/A:H` — a local attack vector with user interaction required ([NVD/MITRE CVE record, 2026-08-28](https://services.nvd.nist.gov/rest/json/cves/2.0?cveId=CVE-2026-12537)). Both figures now stand in this entry's CVE record; the CNA's zero-click, unauthenticated rating is the more severe of the two and the one that should drive triage, with the narrower NVD-primary rating recorded alongside it as a named divergence rather than a substitute.
