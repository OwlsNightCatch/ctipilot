...
Generated from [`config/org-profile.yaml`](../../config/org-profile.yaml) by `tools/compose_prompts.py` — this is the organization the pipeline serves; judge relevance (check 5), watchlist flags, and org-triage / priority calibration (F16) against it, not against a remembered default.
...
**Deployment:** public — entries publish to the open internet. Any closed-source citation marked above TLP:CLEAR is a defect the mechanical gate also FAILs; flag it F7 (drop) with the TLP violation named.
...
- **Dedup context**: `work/<run-id>/prior_coverage.json` (per-entry records for the last 7+ days, including earlier runs today) and `entities/registry.yaml` (canonical entity keys + aliases), plus `state/cves_seen.json`. Use these to spot recycled material shipped as new (should have been `update_of`), wrong update targets, and entity-linking misses (a known alias treated as a new actor).
...
11. **Coverage shape.**
    - **Intel run:** are the `vulnerability` inclusion gates honoured (CISA KEV / EUVD-exploited / EUVD-CVSS-9+ / vendor-or-researcher ITW report / pre-auth-RCE-with-PoC)? Does a deep-dive entry earn its length, and does it respect the ≤1-per-UTC-day budget? Are update-vs-new decisions right — a new entry whose CVEs / entities the prior-coverage index already carries should have been `update_of` (or dropped); an `update_of` entry must carry a genuine delta. Does the rolling 24 h volume stay in the daily band (the run record must justify exceedances)?
...
13. **Missed angles.** Given the dedup context and the run record's source-coverage telemetry, is there a likely-relevant story the research sub-agents probably skipped? Suggest one search query.
