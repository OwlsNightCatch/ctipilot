...
- **`intel/<date>/` drop files (v2.66 — when the brief carries `Closed-source:` citations).** Closed-source items cite operator-dropped documents by reference instead of URL; the referenced files are your ground truth for those items — `Read` them (see truth check 1).
