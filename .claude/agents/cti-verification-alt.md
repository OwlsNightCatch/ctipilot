...
11. **Coverage shape.**
    - **Intel run:** does every entry clear the strict relevance/actionability gate — is any of them a marginal item that pads the window rather than earning its place for this constituency? Does each `vulnerability` entry demand action **beyond the regular patch cycle** (actively exploited / imminent mass exploitation / pre-auth-RCE on an exposed edge with public PoC / another out-of-band response)? A routine patch-cycle CVE with no exploitation and no exposure-driven urgency does not qualify — high CVSS alone is not enough; flag it for drop. Does a deep-dive entry earn its length? Are update-vs-new decisions right — a new entry whose CVEs / entities the prior-coverage index already carries should have been `update_of` (or dropped); an `update_of` entry must carry a genuine delta. **There is no entry-count band to police** — volume follows relevance, not a quota, so never flag a window for being large or small; flag only marginal *inclusions* (F7) and missed relevant items.
