...
**Cross-host redirects are handed back, not followed.** `WebFetch` returns a cross-host redirect URL to you instead of following it, so a source that redirects to another host comes back as a redirect notice rather than content. Re-call `WebFetch` with the returned URL — or, for a redirect chain or an anti-bot host, escalate straight to the jina reader / bridge (both follow redirects from their own egress). A redirect you don't follow is silently-lost content.
...
- **Scope to known primaries — `allowed_domains` / `blocked_domains` first, `site:` as backup.** `WebSearch` takes structured `allowed_domains` and `blocked_domains` arrays: when you know which authority or lab owns a story, pass `allowed_domains: ["cert.ssi.gouv.fr", "msrc.microsoft.com", "<vendor-psirt-host>"]` to force citation-grade results, or `blocked_domains: ["<aggregator-or-seo-farm>"]` to suppress a known-noisy host. The `site:<host>` operator inside the query text is the inline fallback for a single host. Either way you jump straight to the citation-grade page instead of wading through aggregator rewrites.
- **Search in the source language — native reporting leads English by days.** For home-region / sector questions, run the query in German, French and Italian (and the language of any EU jurisdiction in scope), and name the national authority in-language (`NCSC Schweiz Warnung`, `ANSSI alerte`, `BSI Sicherheitswarnung`). A national-CERT advisory or a regional-press disclosure routinely surfaces in-language while the English wire copy is still a day out. Translate the primary inline once you fetch it. **`WebSearch` is US-geolocated**, so European/Swiss pages are not ranked first by default — the in-language query plus `allowed_domains` scoped to the home-region authority is what pulls them up. The curated source list (RSS / bridge / jina, none of them geo-limited) stays your **primary** collection plan for home-region coverage; `WebSearch` is the gap-filler.
...
- **`fetch_method`** — the dispatch switch names the record's PRIMARY transport, but you always follow the **best-content-first ladder and always keep a backup** (§ Fetch tooling reference): `rss` → `python3 tools/fetch_source.py feed <rss_url> [N]`, then `url <link>` per interesting item; `webfetch` → plain `WebFetch` with the outbound-links template; `jina` → `python3 tools/fetch_source.py jina <URL>` (r.jina.ai reader — clean markdown, server-side egress bypasses anti-bot/WAF/geo blocks and runs page JS); `bridge` → `python3 tools/fetch_source.py url <URL>` (browser UA, and it auto-falls-back to the jina reader on a challenge/403 — the direct-WebFetch 403 is expected, not a failure); `api` → the structured subcommand named in the record's `notes`; `blocked` → unreachable by EVERY transport including the reader — `WebSearch` for corroborating coverage and record the gap. When a primary transport fails, escalate down the ladder (feed → WebFetch → `jina` → `url`) before you log a coverage gap; a source is only a `fetch_failures[]` entry when no transport on the ladder returned its content.
...
**The fetch ladder — best content first, always a backup (MANDATORY mental model).** For every source, work down this order and stop at the first rung that returns the real content; if a rung fails or returns thin/blocked/summary-only content, drop to the next. Never treat one failed rung as the end of the road, and never conclude "unreachable" until every rung has failed — every source has a backup transport.
...
2. **Direct `WebFetch`** — the routine's WebFetch tool with the outbound-links template. Fast, but summarises through a small model (URLs dropped without the template; specifics lost on long pages).
3. **jina reader** — `jina <URL>`. The r.jina.ai reader proxy fetches from its OWN egress and executes the page's JavaScript, so it (a) bypasses anti-bot / WAF / geo blocks that 403 our egress (Cloudflare, Akamai, geo-gates), (b) hydrates JS-only SPAs that return an empty shell to a plain GET, and (c) returns the FULL body as clean markdown — so it is also the right rung whenever you need to read a page in complete technical detail rather than a summary.
4. **Dedicated bridge / API recipe** — the browser-UA `url` or the structured subcommand for the host. `url` auto-falls-back to the jina reader on a challenge/403, so it is itself a two-transport rung.
...
- **`feed <URL> [N]`** — parse any RSS/Atom feed → `{title, link, published, summary}` items; auto-falls-back to the jina reader if the direct feed GET is WAF-blocked (`method: jina` in the result). Prefer over `url` whenever a record has `rss_url`; drill interesting items with `url <link>`.
...
- **`jina <URL> [html]`** — force the r.jina.ai reader proxy (rung 3): clean markdown by default, `html` for simplified markup. Use it directly for `jina`-method sources, and reach for it whenever `WebFetch`/`url` return a block, a JS shell, or an over-summarised body you need in full.
...
4. **A JS-empty landing page means escalate the transport** — the jina reader (`jina <URL>`) executes the page's JavaScript and often hydrates the shell into real content in one call; failing that, find the structured endpoint or feed (check the record's notes, probe `https://<host>/sitemap.xml`, look for an RSS path). Never retry the raw shell with the same transport, and never scrape blind when the reader or a structured route gets the content.
