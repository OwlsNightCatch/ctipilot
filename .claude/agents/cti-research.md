...
**When `WebFetch` itself returns thin/empty/blocked content** — a 403, an anti-bot interstitial, a JS-only shell, or prose so summarised the specifics are gone — **do not stop there: escalate down the ladder.** Next rung is the **bridge** (`python3 tools/fetch_source.py url <URL>` — direct browser-UA GET, full raw body; or the host's structured recipe). Only when the direct transports fail too does the **jina reader** (`jina <URL>`) come in — the LAST rung, spending metered API credit: it fetches server-side (bypasses the anti-bot/WAF/geo block that refused our egress) and runs the page's JavaScript (hydrates SPAs), returning clean markdown with the full body; `url` auto-falls-back to it on a challenge/403, so a plain `url` call already covers both rungs. Only after every rung fails is it a coverage gap. When you need a page *in full technical detail* rather than a summary, the right first choice over `WebFetch` is `url <URL>` — the raw body, nothing summarised away — with the reader reserved for hosts the direct fetch cannot read.
...
- **`pdf <URL-or-path>`** — extract the text of a PDF (joint advisories, national-CERT bulletins, regulator decisions, court filings — a large share of authority publications are PDF-first). Stdlib extraction of the document's own text operators, so it spends NO reader credit and returns contiguous prose you can literal-substring-check a quote against. Accepts a local path too, which is the right shape for a heavy document: write it to `work/<run-id>/` once with `url`, then re-read it on disk. It falls back to the reader only when the bytes themselves are refused, warns on stderr when a subset font makes the output unreliable, and fails loudly on a scanned image-only PDF (which needs OCR, not extraction). **A PDF is never a reason to compose from someone else's reading of it** — a 2026-08-20 run concluded an advisory PDF was unreadable, composed the entry from an outlet instead, and its own verification pass then extracted the primary with nothing but a different transport and the standard library.
