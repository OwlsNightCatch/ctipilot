...
- `tools/check_brief.py` `prompt-version` check cross-checks the brief's `**Prompt:** vN.M` badge against the CHANGELOG's newest `## N.M —` heading: FAIL when the brief under check is uncommitted (pre-commit gate), WARN when it's already committed (changelog moved on after publication).
