...
- **Pin:** `attack/enterprise-attack.json` — compact extraction of ONE official ATT&CK Enterprise release (v19.1 at introduction) from `mitre-attack/attack-stix-data` (index.json is the version catalog). Carries id → name, tactics, first-paragraph **definition**, sub-technique parentage, platforms, lifecycle. **Generated file — only `tools/attack_data.py` writes it** (`--check` / `--update [--version X.Y]` / `--selftest` / `--info`). The quality audit runs `--check` on every fire and must record the result (the weekly did until 2026-08-27); update = rewrite + selftest + green build + commit with the printed delta.
- **Revoked ids are kept + forwarded** (`revoked_by` → `content_model.resolve_technique_id`) — the ATT&CK analogue of registry `merged_into` tombstones, needed because the store is not rewritten when the pin moves, so older entries cite old ids forever.
- **Derivation, never assertion:** entity/CVE → technique = union over referencing entries of `content_model.entry_technique_ids` (frontmatter `techniques[]` ∪ dataset-known in-prose T-ids). The 888-entry pre-v3.17 store carries mappings in prose only — prose extraction is a permanent code path, not a migration shim (the tail is not bulk-rewritten; an audit may lift a single entry via an `improvement` record, never the store).
...
**2026-08-27 (v4.0):** the `--check` freshness duty moved from the retired weekly routine to the quality audit (`prompts/quality-audit.md` Phase 0). Entries are living records now ([[entry-lifecycle-v4]]) — a wrong or missing technique id on a published entry is fixed IN that entry through a `correction`/`improvement` changelog record (the mapping feeds the /attack/ matrix and the Navigator exports, so it is exactly the class worth correcting), not by a second entry and not by a bulk rewrite.
