...
- [Classification policy](classification-policy.md) — v3.18: every entry carries exactly one rating (Admiralty, or org_triage only while a triage scheme is configured); check_run FAILs missing ratings + empty behavior-kind techniques[] on v3.18+ runs, --all store-ratings sweep guards it forever
...
- [Scheduler outages & workflow races](scheduler-and-workflow-races.md) — 2026-07-07 62h gap was a CONFIRMED scheduler outage (not repo-side); runaway main runs (17.8h/11.2h) + overtaken-run publish race → v3.21 watchdog + re-sync-and-re-dedup; compose-profile × auto-merge branch-delete race fixed via fallback-to-main; sources.json ensure_ascii drift symptom
