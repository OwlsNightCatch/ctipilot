...
- [Source fetch blocks & primary-source substitutes](source-fetch-blocks.md) — github.com egress-proxy-blocked → OSV.dev bridge; CISA Akamai-blocks every UA (KEV+WebSearch substitute, `bridge-blocked` handled); kernel.org Anubis → distro trackers; JS-only recipe gaps
