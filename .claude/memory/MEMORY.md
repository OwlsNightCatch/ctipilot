...
- [Source fetch blocks & primary-source substitutes](source-fetch-blocks.md) — github.com egress-proxy-blocked → OSV.dev bridge; CISA Akamai-blocks direct fetch but is REACHABLE via `cisa page/feed/csaf` (r.jina.ai reader + cisagov/CSAF mirror); kernel.org Anubis → distro trackers; JS-only recipe gaps
