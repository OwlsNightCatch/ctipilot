...
- [Model identity & verifier rotation](model-identity-and-rotation.md) — CLAUDE_FRIENDLY_NAME/CLAUDE_MODEL_ID are container-scoped and blind to per-agent `model:` pins; uniform sub-agent reports ≠ rotation failure (2026-07-09 run-record claim was a measurement artifact)
...
- [Triage-ready entries](triage-ready-entries.md) — v3.13 actionability shape: observable behavior in vendor-neutral telemetry classes, ATT&CK woven never listed + `techniques[]`/`affected_products[]` frontmatter, `**Triage:**` discriminator omitted-never-invented; migrated_from != null = lower-fidelity tier
