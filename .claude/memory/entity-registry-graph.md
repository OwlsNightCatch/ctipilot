...
- `product` joined the type vocabulary 2026-08-29 and behaves unlike every other type: attached from `affected_products[]` only (NEVER prose phrase-matching), `summary` optional, out of the STIX export, and nothing points out of one except `documented-in`/`related-to` — see [[product-entities]].
